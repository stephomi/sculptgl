define([
  ...
  Import.importAsciiSTL = function (data, mesh) {
    ...
    for (var i = 0; i < nbLength; ++i) {
      ...
      if (line.startsWith('facet')) {
        ...
        iv1 = Import.detectNewVertex(mapVertices, x, y, z, vAr);

        split = lines[i + 3].trim().split(/\s+/);
        x = parseFloat(split[1]);
        y = parseFloat(split[2]);
        z = parseFloat(split[3]);
        iv2 = Import.detectNewVertex(mapVertices, x, y, z, vAr);

        split = lines[i + 4].trim().split(/\s+/);
        x = parseFloat(split[1]);
        y = parseFloat(split[2]);
        z = parseFloat(split[3]);
        iv3 = Import.detectNewVertex(mapVertices, x, y, z, vAr);
  ...
  Import.detectNewVertex = function (mapVertices, x, y, z, vAr) {
    var hash = x + '+' + y + '+' + z;
    var idVertex = mapVertices[hash];
    if (idVertex === undefined) {
      idVertex = vAr.length / 3;
      vAr.push(x, y, z);
  ...
  Import.initMeshArrays = function (mesh, vAr, iAr, cAr) {
    ...
    mesh.indicesABC_ = new Utils.indexArrayType(iAr.length);
