...
Camera.projType.PERSPECTIVE?mat4.lookAt(a,[b,c,this.zoom_],[b,c,0],[0,1,0]):mat4.lookAt(a,[b,c,1E3],[b,c,0],[0,1,0]);var f=mat4.create();mat4.fromQuat(f,this.rot_);mat4.mul(a,a,f);this.usePivot_&&mat4.translate(a,a,vec3.negate([b,c,0],this.center_))},updateProjection:function(){this.proj_=mat4.create();this.type_===Camera.projType.PERSPECTIVE?mat4.perspective(this.proj_,this.fov_*Math.PI/180,this.width_/this.height_,0.01,1E5):this.updateOrtho()},updateTranslation:function(){this.transX_+=this.moveX_*
this.globalScale_/400;this.zoom_=Math.max(1E-5,this.zoom_+this.moveZ_*this.globalScale_/400);this.type_===Camera.projType.ORTHOGRAPHIC&&this.updateOrtho()},translate:function(a,b){this.transX_-=a*this.globalScale_;this.transY_+=b*this.globalScale_},zoom:function(a){this.zoom_=Math.max(1E-5,this.zoom_-a*this.globalScale_);this.type_===Camera.projType.ORTHOGRAPHIC&&this.updateOrtho()},updateOrtho:function(){var a=0.001*Math.abs(this.zoom_);mat4.ortho(this.proj_,-this.width_*a,this.width_*a,-this.height_*
a,this.height_*a,-1E4,1E4)},computePosition:function(){var a=this.view_,b=[-a[12],-a[13],-a[14]],c=mat3.create();mat3.fromMat4(c,a);return vec3.transformMat3(b,b,mat3.transpose(c,c))},reset:function(){this.rot_=quat.create();this.center_=[0,0,0];this.transY_=this.transX_=0},resetViewFront:function(){this.rot_=quat.create()},resetViewTop:function(){this.rot_=quat.rotateX(this.rot_,quat.create(),0.5*Math.PI)},resetViewLeft:function(){this.rot_=quat.rotateY(this.rot_,quat.create(),0.5*-Math.PI)},unproject:function(a,
...
"type_",{Perspective:Camera.projType.PERSPECTIVE,Orthographic:Camera.projType.ORTHOGRAPHIC}).name("Type").onChange(function(a){b.camera_.type_=parseInt(a,10);c.ctrlFov_.__li.hidden=b.camera_.type_===Camera.projType.ORTHOGRAPHIC;b.camera_.updateProjection();b.render()});this.ctrlFov_=f.add(b.camera_,"fov_",10,150).name("Fov");this.ctrlFov_.onChange(function(a){b.camera_.updateProjection();b.render()});f.add(b.camera_,"usePivot_").name("Picking pivot").onChange(function(){b.camera_.center_=[0,0,0];
b.render()});f.open();a=a.addFolder("History");a.add(b,"undo_").name("Undo (Ctrl+Z)");a.add(b,"redo_").name("Redo (Ctrl+Y)");a.open()},initEditingGui:function(a){var b=this.sculptgl_,c=this,f=a.addFolder("Sculpt");this.ctrlSculpt_=f.add(b.sculpt_,"tool_",{"Brush (1)":Sculpt.tool.BRUSH,"Inflate (2)":Sculpt.tool.INFLATE,"Rotate (3)":Sculpt.tool.ROTATE,"Smooth (4)":Sculpt.tool.SMOOTH,"Flatten (5)":Sculpt.tool.FLATTEN,"Pinch (6)":Sculpt.tool.PINCH,"Crease (7)":Sculpt.tool.CREASE,"Drag (8)":Sculpt.tool.DRAG,
