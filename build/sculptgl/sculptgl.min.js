...
Math.min(f,g)),Math.min(c,k)),m=Math.min(Math.min(Math.max(m,e),Math.max(f,g)),Math.max(c,k));return 0<=m&&d<m},intersectSphere:function(a,b){var c=this.min_,d=this.max_,e=a[0],g=a[1],k=a[2],f=[0,0,0];f[0]=c[0]>e?c[0]:d[0]<e?d[0]:e;f[1]=c[1]>g?c[1]:d[1]<g?d[1]:g;f[2]=c[2]>k?c[2]:d[2]<k?d[2]:k;return vec3.sqrDist(a,f)<b},enlargeIfFlat:function(a){var b=this.min_,c=this.max_;b[0]===c[0]&&(b[0]-=a,c[0]+=a);b[1]===c[1]&&(b[1]-=a,c[1]+=a);b[2]===c[2]&&(b[2]-=a,c[2]+=a)},intersectPlane:function(a,b){var c=
this.computeCenter(),c=vec3.dot(vec3.sub(c,c,a),b);return c*c<0.25*vec3.sqrDist(this.min_,this.max_)?2:0<c?1:0}};function Camera(){this.mode_=Camera.mode.PLANE;this.type_=Camera.projType.PERSPECTIVE;this.rot_=quat.create();this.view_=mat4.create();this.proj_=mat4.create();this.lastNormalizedMouseXY_=[0,0];this.height_=this.width_=0;this.zoom_=20;this.transY_=this.transX_=0;this.speed_=1;this.moveZ_=this.moveX_=0;this.fov_=70;this.center_=[0,0,0];this.stepCenter_=[0,0,0];this.stepCount_=this.stepZoom_=0;this.usePivot_=!1}Camera.mode={SPHERICAL:0,PLANE:1};Camera.projType={PERSPECTIVE:0,ORTHOGRAPHIC:1};
...
this.transX_=0},reset:function(){this.transY_=this.transX_=0;this.speed_=1.4*Mesh.globalScale_;this.rot_=quat.create();this.center_=[0,0,0];this.zoom_=0;this.zoom(-0.4)},resetViewFront:function(){this.rot_=quat.create()},resetViewTop:function(){this.rot_=quat.rotateX(this.rot_,quat.create(),0.5*Math.PI)},resetViewLeft:function(){this.rot_=quat.rotateY(this.rot_,quat.create(),0.5*-Math.PI)},unproject:function(a,b,c){var d=this.height_;a=[2*a/this.width_-1,(d-2*b)/d,2*c-1,1];b=mat4.create();vec4.transformMat4(a,
a,mat4.invert(b,mat4.mul(b,this.proj_,this.view_)));b=a[3];return[a[0]/b,a[1]/b,a[2]/b]},project:function(a){a=[a[0],a[1],a[2],1];vec4.transformMat4(a,a,this.view_);vec4.transformMat4(a,a,this.proj_);var b=a[3],c=this.height_;return[0.5*(a[0]/b+1)*this.width_,c-0.5*(a[1]/b+1)*c,0.5*(a[2]/b+1)]}};var Import={importOBJ:function(a,b){for(var c=b.vertices_,d=b.triangles_,e=[],g=[],k=a.split("\n"),f=[],h=k.length,l=0;l<h;++l){var m=k[l].trim();if(m.startsWith("v "))f=m.split(/\s+/),c.push(new Vertex(c.length)),e.push(parseFloat(f[1]),parseFloat(f[2]),parseFloat(f[3]));else if(m.startsWith("f ")){var f=m.split(/\s+/),m=f[1].split("/"),n=f[2].split("/"),p=f[3].split("/"),m=parseInt(m[0],10),q=parseInt(n[0],10),p=parseInt(p[0],10),r=c.length;0>m?(m+=r,q+=r,p+=r):(--m,--q,--p);var n=c[m],s=c[q],t=
c[p],v=d.length;n.tIndices_.push(v);s.tIndices_.push(v);t.tIndices_.push(v);d.push(new Triangle(v));g.push(m,q,p);4<f.length&&(++v,f=parseInt(f[4].split("/")[0],10),0>f?f+=r:--f,q=c[f],n.tIndices_.push(v),t.tIndices_.push(v),q.tIndices_.push(v),d.push(new Triangle(v)),g.push(m,p,f))}}Import.initMeshArrays(b,e,g)},importPLY:function(a,b){for(var c=b.vertices_,d=b.triangles_,e=[],g=[],k=[],f=a.split("\n"),h=[],l=-1,m=-1,n=-1,p=0;;){h=f[p].trim();if(h.startsWith("element vertex ")){for(var h=h.split(/\s+/),
l=parseInt(h[2],10),q=p;;)if(++p,h=f[p].trim(),h.startsWith("property ")){if(h=h.split(/\s+/),"red"===h[2]){n=p-q-1;break}}else break;--p}else if(h.startsWith("element face "))h=h.split(/\s+/),m=parseInt(h[2],10);else if(h.startsWith("end_header")){++p;l+=p;for(q=1/255;p<l;++p)h=f[p].trim(),h=h.split(/\s+/),c.push(new Vertex(c.length)),e.push(parseFloat(h[0]),parseFloat(h[1]),parseFloat(h[2])),g.push(parseInt(h[n],10)*q,parseInt(h[n+1],10)*q,parseInt(h[n+2],10)*q);for(m+=p;p<m;++p){var h=f[p].trim(),
h=h.split(/\s+/),r=parseInt(h[0],10);if(3===r||4===r){var n=parseInt(h[1],10),s=parseInt(h[2],10),l=parseInt(h[3],10),q=c[n],t=c[s],v=c[l],w=d.length;q.tIndices_.push(w);t.tIndices_.push(w);v.tIndices_.push(w);d.push(new Triangle(w));k.push(n,s,l);4===r&&(++w,h=parseInt(h[4],10),r=c[h],q.tIndices_.push(w),v.tIndices_.push(w),r.tIndices_.push(w),d.push(new Triangle(w)),k.push(n,l,h))}}break}++p}Import.initMeshArrays(b,e,k,g)},importSTL:function(a,b){84+50*Import.getUint32(a,80)===a.length?Import.importBinarySTL(a,
...
exportSketchfab:function(a,b){var c=new FormData;c.append("token",b);var d=Export.exportOBJ(a);c.append("fileModel",new Blob([d]));c.append("filenameModel","model.obj");var e=new XMLHttpRequest;e.open("POST","https://api.sketchfab.com/v1/models");e.addEventListener("load",function(){var a=JSON.parse(e.responseText);console.log(a);a.success?alert("Upload success !"):alert("Sketchfab upload error :\n"+a.error)},!0);e.send(c)}};var Geometry={normalizedMouse:function(a,b,c,d){return[2*a/c-1,1-2*b/d]},mouseOnUnitSphere:function(a){var b=a[0];a=a[1];var c=1-b*b-a*a,c=0<c?Math.sqrt(c):0,b=[b,a,c];return vec3.normalize(b,b)},intersectionRayTriangle:function(a,b,c,d,e,g,k){var f=[0,0,0],h=vec3.dot(vec3.sub(f,a,c),g),l=vec3.dot(vec3.sub(f,b,c),g);if(0<=h*l)return!1;vec3.scaleAndAdd(k,a,vec3.sub(f,b,a),-h/(l-h));a=[0,0,0];vec3.cross(a,g,vec3.sub(f,d,c));if(0>vec3.dot(a,vec3.sub(f,k,c)))return!1;vec3.cross(a,g,vec3.sub(f,e,d));if(0>
vec3.dot(a,vec3.sub(f,k,d)))return!1;vec3.cross(a,g,vec3.sub(f,c,e));return 0>vec3.dot(a,vec3.sub(f,k,c))?!1:!0},computeTriangleAabb:function(a,b,c,d,e,g,k,f,h,l){var m=Math.min(b,e,f),n=Math.min(c,g,h),p=Math.min(d,k,l);b=Math.max(b,e,f);c=Math.max(c,g,h);d=Math.max(d,k,l);k=a.min_;a=a.max_;k[0]=m;k[1]=n;k[2]=p;a[0]=b;a[1]=c;a[2]=d},triangleNormal:function(a,b,c,d,e,g,k,f,h,l){e-=b;g-=c;k-=d;b=f-b;c=h-c;f=l-d;h=l=d=0;d=g*f-k*c;l=k*b-e*f;h=e*c-g*b;e=d*d+l*l+h*h;0!==e&&(e=1/Math.sqrt(e),a[0]=d*e,a[1]=
l*e,a[2]=h*e)},pointInsideTriangle:function(a,b,c,d){var e=[0,0,0];vec3.sub(e,b,c);var g=[0,0,0];vec3.sub(g,b,d);b=[0,0,0];vec3.sub(b,a,c);c=[0,0,0];vec3.sub(c,a,d);d=[0,0,0];a=vec3.len(vec3.cross(d,e,g));e=vec3.len(vec3.cross(d,e,b));g=vec3.len(vec3.cross(d,g,c));b=vec3.len(vec3.cross(d,b,c));return 1E-20>Math.abs(a-(e+g+b))},sphereIntersectTriangle:function(a,b,c,d,e){return Geometry.distanceSqToSegment(a,c,d)<b||Geometry.distanceSqToSegment(a,d,e)<b||Geometry.distanceSqToSegment(a,c,e)<b?!0:!1},
distanceSqToSegment:function(a,b,c){var d=[0,0,0];vec3.sub(d,a,b);var e=[0,0,0];vec3.sub(e,c,b);var g=vec3.sqrLen(e),g=vec3.dot(d,e)/g;if(0>g)return vec3.sqrLen(d);if(1<g)return vec3.sqrLen(vec3.sub(d,a,c));d[0]=a[0]-b[0]+g*e[0];d[1]=a[1]-b[1]+g*e[1];d[2]=a[2]-b[2]+g*e[2];return vec3.sqrLen(d)},signedAngle2d:function(a,b){var c=a[0],d=a[1],e=b[0],g=b[1];return Math.atan2(c*g-d*e,c*e+d*g)},pointPlaneDistance:function(a,b,c){return vec3.dot(vec3.sub([0,0,0],a,b),c)},mirrorPoint:function(a,b,c){return vec3.sub(a,
a,vec3.scale([0,0,0],c,2*Geometry.pointPlaneDistance(a,b,c)))},vertexOnLine:function(a,b,c){var d=[0,0,0];vec3.sub(d,c,b);c=[0,0,0];a=vec3.dot(d,vec3.sub(c,a,b));return vec3.scaleAndAdd(c,b,d,a/vec3.sqrLen(d))},intersectLinePlane:function(a,b,c,d,e){void 0===e&&(e=[0,0,0]);var g=vec3.dot(vec3.sub(e,a,c),d);c=vec3.dot(vec3.sub(e,b,c),d);return g===c?b:vec3.scaleAndAdd(e,a,vec3.sub(e,b,a),-g/(c-g))},getPerpendicularVector:function(a){var b=[0,0,0];0===a[0]?b[0]=1:0===a[1]?b[1]=1:0===a[2]?b[2]=1:(b[0]=
a[2],b[1]=a[2],b[2]=-a[0]-a[1],vec3.normalize(b,b));return b}};function Grid(){this.aabb_=new Aabb;this.size_=this.cellSize_=this.dimZ_=this.dimY_=this.dimX_=0;this.iVerts_=[]}
...
Octree.maxDepth_)this.constructCells(a);else for(h=0;h<b;++h)l=g[c[h]],l.tagFlag_=f,l.leaf_=this,l.posInLeaf_=h},constructCells:function(a){var b=this.child_,c=this.iTris_,d=this.aabbSplit_.min_,e=this.aabbSplit_.max_,g=d[0],k=d[1],f=d[2],h=e[0],l=e[1],m=e[2],n=this.aabbSplit_.computeCenter(),p=n[0],q=n[1],r=n[2],s=0.5*(h-g),t=0.5*(l-k),v=0.5*(m-f),w=this.depth_+1,u=new Aabb;b[0]=new Octree(this,w);b[0].build(a,c,u.setCopy(d,n));b[1]=new Octree(this,w);b[1].build(a,c,u.set(g+s,k,f,p+s,q,r));b[2]=
new Octree(this,w);b[2].build(a,c,u.set(p,q-t,r,h,l-t,m));b[3]=new Octree(this,w);b[3].build(a,c,u.set(g,k,f+v,p,q,r+v));b[4]=new Octree(this,w);b[4].build(a,c,u.set(g,k+t,f,p,q+t,r));b[5]=new Octree(this,w);b[5].build(a,c,u.set(p,q,r-v,h,l,m-v));b[6]=new Octree(this,w);b[6].build(a,c,u.setCopy(n,e));b[7]=new Octree(this,w);b[7].build(a,c,u.set(p-s,q,r,h-s,l,m));this.iTris_.length=0},intersectRay:function(a,b){if(this.aabbLoose_.intersectRay(a,b)){if(this.child_[0]){for(var c=[],d=this.child_,e=0;8>
e;++e){var g=d[e].intersectRay(a,b);c.push.apply(c,g)}return c}return this.iTris_}return[]},intersectSphere:function(a,b,c){if(this.aabbSplit_.intersectSphere(a,b)){if(this.child_[0]){for(var d=[],e=this.child_,g=0;8>g;++g){var k=e[g].intersectSphere(a,b,c);d.push.apply(d,k)}return d}c.push(this);return this.iTris_}return[]},cullPlane:function(a,b,c,d){var e=0;if(e=this.aabbSplit_.intersectPlane(a,b))if(this.child_[0])for(var e=this.child_,g=0;8>g;++g)e[g].cullPlane(a,b,c,d);else 1===e?c.push.apply(c,
this.iTris_):d.push.apply(d,this.iTris_)},addTriangle:function(a,b){if(this.aabbSplit_.pointInside(b.aabb_.computeCenter())){this.aabbLoose_.expandsWithAabb(b.aabb_);var c=this.child_;if(c[0])for(var d=0;8>d;++d)c[d].addTriangle(a,b);else b.posInLeaf_=this.iTris_.length,b.leaf_=this,this.iTris_.push(b.id_)}}};
Octree.checkEmptiness=function(a,b){var c=a.parent_;if(c){for(var d=c.child_,e=0,e=0;8>e;++e)if(0<d[e].iTris_.length||d[e].child_[0])return;b.push(c);for(e=0;8>e;++e)b.push(d[e]);Octree.checkEmptiness(c,b)}};function Picking(a){this.mesh_=null;this.pickedTriangle_=-1;this.pickedVertices_=[];this.interPoint_=[0,0,0];this.rDisplay_=50;this.rWorldSqr_=0;this.camera_=a;this.eyeDir_=[0,0,0]}
...
a.indexArray_,l=[0,0,0];vec3.sub(l,c,b);vec3.normalize(l,l);for(var l=a.octree_.intersectRay(b,[1/l[0],1/l[1],1/l[2]]),m=-1,n=l.length,p=[0,0,0],q=0;q<n;++q){var r=l[q],s=k[r],r=3*r,t=3*h[r],v=3*h[r+1],r=3*h[r+2];if(Geometry.intersectionRayTriangle(b,c,[f[t],f[t+1],f[t+2]],[f[v],f[v+1],f[v+2]],[f[r],f[r+1],f[r+2]],s.normal_,p)&&(s=vec3.sqrDist(b,p),s<m||-1===m))m=s,vec3.copy(this.interPoint_,p),this.pickedTriangle_=l[q]}-1!==this.pickedTriangle_?(this.mesh_=a,this.computeRadiusWorldSq(d,e,g)):this.rWorldSqr_=
...
f=3*this.pickedTriangle_,this.pickedVertices_.push(a[f],a[f+1],a[f+2]))},computeRadiusWorldSq:function(a,b,c){var d=[0,0,0];vec3.transformMat4(d,this.interPoint_,this.mesh_.matTransform_);var e=this.camera_.project(d)[2];a=this.camera_.unproject(a+this.rDisplay_*c,b,e);this.rWorldSqr_=vec3.sqrDist(d,a)}};function Render(a){this.gl_=a;this.shaderType_=Render.mode.PHONG;this.reflectionTexUnif_=this.lineNormalUnif_=this.lineOriginUnif_=this.radiusSquaredUnif_=this.centerPickingUnif_=this.normalMatrixUnif_=this.mvMatrixUnif_=this.mvpMatrixUnif_=this.barycenterAttrib_=this.normalAttrib_=this.colorAttrib_=this.vertexAttrib_=this.vertexShader_=this.fragmentShader_=this.shaderProgram_=this.reflectionLoc_=this.indexBuffer_=this.barycenterBuffer_=this.colorBuffer_=this.normalBuffer_=this.vertexBuffer_=null}
...
this.lineNormalUnif_=b.getUniformLocation(a,"lineNormal");this.shaderType_>=Render.mode.MATERIAL&&(this.reflectionTexUnif_=b.getUniformLocation(a,"refTex"));b.detachShader(a,this.fragmentShader_);b.deleteShader(this.fragmentShader_);b.detachShader(a,this.vertexShader_);b.deleteShader(this.vertexShader_)},loadShaders:function(a,b){var c=this.gl_;this.vertexShader_=c.createShader(c.VERTEX_SHADER);c.shaderSource(this.vertexShader_,"\n"+a+"\n");c.compileShader(this.vertexShader_);this.fragmentShader_=
c.createShader(c.FRAGMENT_SHADER);c.shaderSource(this.fragmentShader_,b);c.compileShader(this.fragmentShader_)},initBuffers:function(a,b,c,d){var e=this.gl_;this.vertexBuffer_=e.createBuffer();e.bindBuffer(e.ARRAY_BUFFER,this.vertexBuffer_);e.bufferData(e.ARRAY_BUFFER,a,e.DYNAMIC_DRAW);this.normalBuffer_=e.createBuffer();e.bindBuffer(e.ARRAY_BUFFER,this.normalBuffer_);e.bufferData(e.ARRAY_BUFFER,b,e.DYNAMIC_DRAW);this.colorBuffer_=e.createBuffer();e.bindBuffer(e.ARRAY_BUFFER,this.colorBuffer_);e.bufferData(e.ARRAY_BUFFER,
c,e.DYNAMIC_DRAW);this.indexBuffer_=e.createBuffer();e.bindBuffer(e.ELEMENT_ARRAY_BUFFER,this.indexBuffer_);e.bufferData(e.ELEMENT_ARRAY_BUFFER,d,e.STATIC_DRAW)},render:function(a,b,c,d,e,g,k){e=this.gl_;e.useProgram(this.shaderProgram_);var f=b.interPoint_;b=b.rWorldSqr_;var h=mat4.create();mat4.mul(h,a.view_,c);c=mat4.create();mat4.mul(c,a.proj_,h);e.enableVertexAttribArray(this.vertexAttrib_);e.bindBuffer(e.ARRAY_BUFFER,this.vertexBuffer_);e.vertexAttribPointer(this.vertexAttrib_,3,e.FLOAT,!1,
0,0);e.enableVertexAttribArray(this.normalAttrib_);e.bindBuffer(e.ARRAY_BUFFER,this.normalBuffer_);e.vertexAttribPointer(this.normalAttrib_,3,e.FLOAT,!1,0,0);this.shaderType_!==Render.mode.WIREFRAME&&this.shaderType_!==Render.mode.NORMAL&&(e.enableVertexAttribArray(this.colorAttrib_),e.bindBuffer(e.ARRAY_BUFFER,this.colorBuffer_),e.vertexAttribPointer(this.colorAttrib_,3,e.FLOAT,!1,0,0));e.uniformMatrix4fv(this.mvMatrixUnif_,!1,h);e.uniformMatrix4fv(this.mvpMatrixUnif_,!1,c);e.uniformMatrix3fv(this.normalMatrixUnif_,
!1,mat3.normalFromMat4(mat3.create(),h));e.uniform3fv(this.centerPickingUnif_,vec3.transformMat4([0,0,0],f,h));e.uniform1f(this.radiusSquaredUnif_,b);e.uniform2fv(this.lineOriginUnif_,g);e.uniform2fv(this.lineNormalUnif_,k);switch(this.shaderType_){case Render.mode.PHONG:this.drawBuffer(d);break;case Render.mode.TRANSPARENCY:e.depthMask(!1);e.enable(e.BLEND);this.drawBuffer(d);e.disable(e.BLEND);e.depthMask(!0);break;case Render.mode.WIREFRAME:e.enableVertexAttribArray(this.barycenterAttrib_);e.bindBuffer(e.ARRAY_BUFFER,
this.barycenterBuffer_);e.vertexAttribPointer(this.barycenterAttrib_,3,e.FLOAT,!1,0,0);e.drawArrays(e.TRIANGLES,0,d);break;case Render.mode.NORMAL:this.drawBuffer(d);break;default:e.activeTexture(e.TEXTURE0),e.bindTexture(e.TEXTURE_2D,this.reflectionLoc_),e.uniform1i(this.reflectionTexUnif_,0),this.drawBuffer(d)}},drawBuffer:function(a){var b=this.gl_;b.bindBuffer(b.ELEMENT_ARRAY_BUFFER,this.indexBuffer_);b.drawElements(b.TRIANGLES,a,SculptGL.elementIndexType,0)},updateBuffers:function(a,b,c,d){if(this.shaderType_===
Render.mode.WIREFRAME)this.makeWireframeBuffers(a,b,d);else{var e=this.gl_;e.bindBuffer(e.ARRAY_BUFFER,this.vertexBuffer_);e.bufferData(e.ARRAY_BUFFER,a,e.DYNAMIC_DRAW);e.bindBuffer(e.ARRAY_BUFFER,this.normalBuffer_);e.bufferData(e.ARRAY_BUFFER,b,e.DYNAMIC_DRAW);e.bindBuffer(e.ARRAY_BUFFER,this.colorBuffer_);e.bufferData(e.ARRAY_BUFFER,c,e.DYNAMIC_DRAW);e.bindBuffer(e.ELEMENT_ARRAY_BUFFER,this.indexBuffer_);e.bufferData(e.ELEMENT_ARRAY_BUFFER,d,e.STATIC_DRAW)}},makeWireframeBuffers:function(a,b,c){for(var d=
this.gl_,e=new Float32Array(3*c.length),g=0,k=0,f=0,g=0;g<c.length;++g)k=3*g,f=3*c[g],e[k]=a[f],e[k+1]=a[f+1],e[k+2]=a[f+2];d.bindBuffer(d.ARRAY_BUFFER,this.vertexBuffer_);d.bufferData(d.ARRAY_BUFFER,e,d.DYNAMIC_DRAW);a=new Float32Array(3*c.length);for(g=0;g<c.length;++g)k=3*g,f=3*c[g],a[k]=b[f],a[k+1]=b[f+1],a[k+2]=b[f+2];d.bindBuffer(d.ARRAY_BUFFER,this.normalBuffer_);d.bufferData(d.ARRAY_BUFFER,a,d.DYNAMIC_DRAW);b=new Float32Array(3*c.length);for(g=0;g<c.length/3;++g)k=9*g,b[k]=1,b[k+1]=0,b[k+
2]=0,b[k+3]=0,b[k+4]=1,b[k+5]=0,b[k+6]=0,b[k+7]=0,b[k+8]=1;this.barycenterBuffer_||(this.barycenterBuffer_=d.createBuffer());d.bindBuffer(d.ARRAY_BUFFER,this.barycenterBuffer_);d.bufferData(d.ARRAY_BUFFER,b,d.DYNAMIC_DRAW)}};function Sculpt(a){this.states_=a;this.mesh_=null;this.intensity_=0.75;this.tool_=Sculpt.tool.BRUSH;this.topo_=Sculpt.topo.SUBDIVISION;this.detailSubdivision_=0.75;this.detailDecimation_=0.1;this.negative_=!1;this.clay_=!0;this.culling_=!1;this.color_=[168,66,66];this.d2Max_=this.d2Min_=0;this.d2Thickness_=0.5;this.d2Move_=0;this.rotateData_={normal:[0,0,0],center:[0,0]};this.rotateDataSym_={normal:[0,0,0],center:[0,0]};this.dragDir_=[0,0,0];this.dragDirSym_=[0,0,0];this.lineOrigin_=[0,0];this.lineNormal_=
[0,0];this.fillHoles_=!0}Sculpt.tool={BRUSH:0,INFLATE:1,ROTATE:2,SMOOTH:3,FLATTEN:4,PINCH:5,CREASE:6,DRAG:7,COLOR:8,SCALE:9,CUT:10};Sculpt.topo={STATIC:0,SUBDIVISION:1,ADAPTIVE:2};
...
this.d2Max_),p=q.decimation(p,this.d2Min_)}for(var h=f.getVerticesFromTriangles(p),r=h.length,s=[],t=[],v=Vertex.sculptMask_,w=f.normalArray_,u=n[0],x=n[1],n=n[2],y=0;y<r;++y){var A=h[y];if(a[A].sculptFlag_===v){s.push(A);var z=3*A;0>=w[z]*u+w[z+1]*x+w[z+2]*n&&t.push(A)}}if(0!==t.length)switch(this.culling_&&(s=t),this.tool_){case Sculpt.tool.BRUSH:this.brush(m,s,t,l,b);break;case Sculpt.tool.INFLATE:this.inflate(m,s,l,b);break;case Sculpt.tool.ROTATE:this.rotate(m,s,l,d,e,g,k,c);break;case Sculpt.tool.SMOOTH:this.smooth(s,
...
Math.sqrt(c);f=b.length;e=a[0];for(var l=a[1],m=a[2],n=0;n<f;++n){var p=3*b[n],q=k[p]-e,r=k[p+1]-l,s=k[p+2]-m,q=Math.sqrt(q*q+r*r+s*s)/c,r=[k[p],k[p+1],k[p+2]];quat.setAxisAngle(d,h,g*(3*q*q*q*q-4*q*q*q+1));vec3.sub(r,r,a);vec3.transformQuat(r,r,d);vec3.add(r,r,a);k[p]=r[0];k[p+1]=r[1];k[p+2]=r[2]}}},smooth:function(a,b){var c=this.mesh_.vertexArray_,d=a.length,e=new Float32Array(3*d);this.laplacianSmooth(a,e);for(var g=this.d2Move_,k=Math.sqrt(g),f=this.topo_===Sculpt.topo.ADAPTIVE,h=0;h<d;++h){var l=
3*a[h],m=3*h,n=(e[m]-c[l])*b,p=(e[m+1]-c[l+1])*b,m=(e[m+2]-c[l+2])*b;if(f){var q=n*n+p*p+m*m;q>g&&(q=k/Math.sqrt(q),n*=q,p*=q,m*=q)}c[l]+=n;c[l+1]+=p;c[l+2]+=m}},flatten:function(a,b,c,d,e){var g=this.areaNormal(c);if(g){var k=this.areaCenter(c);c=this.mesh_.vertexArray_;d=Math.sqrt(d);var f=b.length;e*=0.3;var h=a[0],l=a[1];a=a[2];for(var m=k[0],n=k[1],k=k[2],p=g[0],q=g[1],g=g[2],r=Math.sqrt(this.d2Move_),s=this.topo_===Sculpt.topo.ADAPTIVE,t=0;t<f;++t){var v=3*b[t],w=c[v],u=c[v+1],x=c[v+2],y=(w-
m)*p+(u-n)*q+(x-k)*g,w=w-h,u=u-l,x=x-a,x=Math.sqrt(w*w+u*u+x*x)/d,x=3*x*x*x*x-4*x*x*x+1,x=y*e*x;s&&x>r&&(x=r);c[v]-=p*x;c[v+1]-=q*x;c[v+2]-=g*x}}},pinch:function(a,b,c,d){var e=this.mesh_.vertexArray_;c=Math.sqrt(c);var g=b.length,k=a[0],f=a[1];a=a[2];var h=this.d2Move_,l=Math.sqrt(h),m=this.topo_===Sculpt.topo.ADAPTIVE;d=0.005*d*c;for(var n=0;n<g;++n){var p=3*b[n],q=k-e[p],r=f-e[p+1],s=a-e[p+2],t=Math.sqrt(q*q+r*r+s*s)/c,t=3*t*t*t*t-4*t*t*t+1,t=d*t,q=q*t,r=r*t,s=s*t;m&&(t=q*q+r*r+s*s,t>h&&(t=l/Math.sqrt(t),
q*=t,r*=t,s*=t));e[p]+=q;e[p+1]+=r;e[p+2]+=s}},crease:function(a,b,c,d,e){var g=this.areaNormal(c);if(g){c=this.mesh_.vertexArray_;d=Math.sqrt(d);var k=b.length,f=a[0],h=a[1];a=a[2];var l=g[0],m=g[1],g=g[2],n=this.d2Move_,p=Math.sqrt(n),q=this.topo_===Sculpt.topo.ADAPTIVE;e=0.005*e*d;var r=10;this.negative_&&(r=-10);for(var s=0;s<k;++s){var t=3*b[s],v=f-c[t],w=h-c[t+1],u=a-c[t+2],x=Math.sqrt(v*v+w*w+u*u)/d,x=3*x*x*x*x-4*x*x*x+1,y=e*Math.pow(2-x,-5)*r,x=0.1*x,v=v*x+l*y,w=w*x+m*y,u=u*x+g*y;q&&(x=v*
v+w*w+u*u,x>n&&(x=p/Math.sqrt(x),v*=x,w*=x,u*=x));c[t]+=v;c[t+1]+=w;c[t+2]+=u}}},updateDragDir:function(a,b,c,d,e,g,k){var f=b.camera_.unproject(c,d,0),h=b.camera_.unproject(c,d,1),l=mat4.create();mat4.invert(l,a.matTransform_);vec3.transformMat4(f,f,l);vec3.transformMat4(h,h,l);l=this.dragDir_;g&&(l=this.dragDirSym_,Geometry.mirrorPoint(f,g,k),Geometry.mirrorPoint(h,g,k));g=b.interPoint_;b.interPoint_=Geometry.vertexOnLine(g,f,h);vec3.sub(l,b.interPoint_,g);b.mesh=a;b.computeRadiusWorldSq(c,d,e);
a=b.eyeDir_;vec3.sub(a,h,f);vec3.normalize(a,a)},drag:function(a,b,c,d){var e=this.mesh_.vertexArray_,g=b.length;c=Math.sqrt(c);var k=a[0],f=a[1];a=a[2];var h=d?this.dragDirSym_:this.dragDir_;d=h[0];for(var l=h[1],h=h[2],m=0;m<g;++m){var n=3*b[m],p=e[n]-k,q=e[n+1]-f,r=e[n+2]-a,p=Math.sqrt(p*p+q*q+r*r)/c,p=3*p*p*p*p-4*p*p*p+1;e[n]+=d*p;e[n+1]+=l*p;e[n+2]+=h*p}},paint:function(a,b,c){var d=this.mesh_,e=d.vertexArray_,d=d.colorArray_,g=this.color_;c=Math.sqrt(c);var k=g[0]/255,f=g[1]/255,g=g[2]/255,
h=a[0],l=a[1];a=a[2];for(var m=this.intensity_,n=b.length,p=0;p<n;++p){var q=3*b[p],r=e[q]-h,s=e[q+1]-l,t=e[q+2]-a,r=Math.sqrt(r*r+s*s+t*t)/c,r=3*r*r*r*r-4*r*r*r+1,r=r*m,s=1-r;d[q]=d[q]*s+k*r;d[q+1]=d[q+1]*s+f*r;d[q+2]=d[q+2]*s+g*r}},smoothFlat:function(a,b){var c=this.mesh_,d=c.vertexArray_,c=c.normalArray_,e=a.length,g=new Float32Array(3*e);this.laplacianSmooth(a,g);for(var k=0;k<e;++k){var f=3*a[k],h=d[f],l=d[f+1],m=d[f+2],n=c[f],p=c[f+1],q=c[f+2],r=3*k,s=g[r],t=g[r+1],r=g[r+2],v=n*(s-h)+p*(t-
l)+q*(r-m);d[f]+=(s-n*v-h)*b;d[f+1]+=(t-p*v-l)*b;d[f+2]+=(r-q*v-m)*b}},laplacianSmooth:function(a,b){for(var c=this.mesh_,d=c.vertices_,c=c.vertexArray_,e=a.length,g=0;g<e;++g){var k=3*g,f=d[a[g]],h=f.ringVertices_,l=h.length,m=0,n=0,p=0,q=0,r=0;if(l!==f.tIndices_.length){for(q=f=0;q<l;++q){var r=h[q],s=d[r];s.ringVertices_.length!==s.tIndices_.length&&(r*=3,m+=c[r],n+=c[r+1],p+=c[r+2],++f)}b[k]=m/f;b[k+1]=n/f;b[k+2]=p/f}else{for(q=0;q<l;++q)r=3*h[q],m+=c[r],n+=c[r+1],p+=c[r+2];b[k]=m/l;b[k+1]=n/
l;b[k+2]=p/l}}},areaNormal:function(a){for(var b=this.mesh_.normalArray_,c=a.length,d=0,e=0,g=0,k=0;k<c;++k)var f=3*a[k],d=d+b[f],e=e+b[f+1],g=g+b[f+2];a=Math.sqrt(d*d+e*e+g*g);if(0===a)return null;a=1/a;return[d*a,e*a,g*a]},areaCenter:function(a){for(var b=this.mesh_.vertexArray_,c=a.length,d=0,e=0,g=0,k=0;k<c;++k)var f=3*a[k],d=d+b[f],e=e+b[f+1],g=g+b[f+2];return[d/c,e/c,g/c]},setLineOrigin:function(a,b){this.lineOrigin_=[a,b]},updateLineNormal:function(a,b){this.lineNormal_=[this.lineOrigin_[1]-
...
c,d);b.updateBuffers();this.lineNormal_=[0,0];this.lineNormal_=[0,0]},uniformProjection:function(a,b,c){var d=this.mesh_,e=d.vertexArray_,g=d.vertices_,k=[],f=[];d.octree_.cullPlane(a,b,k,f);var h=d.getVerticesFromTriangles(k),l=d.getVerticesFromTriangles(f);this.states_.pushState(k,h);this.states_.pushState(f,l);f=k=0;++Vertex.tagMask_;for(var m=Vertex.tagMask_,f=h.length,k=0;k<f;++k)g[h[k]].tagFlag_=m;for(var n=[0,0,0],p=l.length,k=0;k<p;++k)f=l[k],g[f].tagFlag!==m&&(n[0]=e[3*f],n[1]=e[3*f+1],n[2]=
e[3*f+2],0<vec3.dot(b,vec3.sub(n,n,a))&&h.push(f));g=d.getTrianglesFromVertices(h);e=new Topology(this.states_);e.mesh_=d;e.planeOrigin_=a;e.planeNormal_=b;e.checkPlane_=!0;e.linearSubdivision_=!0;this.setAdaptiveParameters(e.radiusSquared_);c=Math.max(Math.min(c,500),10);g=e.subdivision(g,c);g=e.decimation(g,c/4.2025);e=d.vertexArray_;c=d.getVerticesFromTriangles(g);h=c.length;l=a[0];m=a[1];a=a[2];n=b[0];p=b[1];b=b[2];var q=new Float32Array(3*h);this.laplacianSmooth(c,q);for(k=0;k<h;++k){var f=3*
c[k],r=(e[f]-l)*n+(e[f+1]-m)*p+(e[f+2]-a)*b;if(!(0>r)){var f=3*k,s=q[f],t=q[f+1],v=q[f+2],r=(s-l)*n+(t-m)*p+(v-a)*b,f=3*c[k];e[f]=s-n*r;e[f+1]=t-p*r;e[f+2]=v-b*r}}d.updateMesh(g,d.getVerticesFromTriangles(g))}};var Utils={nextHighestPowerOfTwo:function(a){--a;for(var b=1;32>b;b<<=1)a|=a>>b;return a+1},tidy:function(a){a.sort(function(a,b){return a-b});for(var b=a.length,c=0,d=0,c=1;c<b;++c)a[d]!==a[c]&&(a[++d]=a[c]);1<c&&(a.length=d+1)},intersectionArrays:function(a,b){for(var c=0,d=0,e=[],g=a.length,k=b.length;c<g&&d<k;)a[c]<b[d]?c++:a[c]>b[d]?d++:(e.push(a[c]),++c,++d);return e},rotateArray:function(a,b){b=-b;var c=a.length;b=(Math.abs(b)>=c&&(b%=c),0>b&&(b+=c),b);for(var d,e;b;b=(Math.ceil(c/b)-1)*b-
...
Topology.prototype.findSplit=function(a,b,c){var d=this.mesh_,e=d.vertexArray_,d=d.indexArray_,g=3*a;a=d[g+1];var k=d[g+2],g=3*d[g],d=[e[g],e[g+1],e[g+2]],g=3*a;a=[e[g],e[g+1],e[g+2]];g=3*k;e=[e[g],e[g+1],e[g+2]];if(this.checkPlane_){if(c=this.planeOrigin_,g=this.planeNormal_,k=[0,0,0],0>vec3.dot(g,vec3.sub(k,d,c))&&0>vec3.dot(g,vec3.sub(k,a,c))&&0>vec3.dot(g,vec3.sub(k,e,c)))return 0}else if(c&&!Geometry.sphereIntersectTriangle(this.center_,2*this.radiusSquared_,d,a,e)&&!Geometry.pointInsideTriangle(this.center_,
d,a,e))return 0;g=[0,0,0];c=vec3.sqrLen(vec3.sub(g,d,a));a=vec3.sqrLen(vec3.sub(g,a,e));d=vec3.sqrLen(vec3.sub(g,d,e));return c>a&&c>d?c>b?1:0:a>d?a>b?2:0:d>b?3:0};
...
Topology.prototype.halfEdgeSplit=function(a,b,c,d){var e=this.mesh_,g=e.vertices_,k=e.triangles_,f=e.vertexArray_,h=e.normalArray_,l=e.colorArray_,m=e.indexArray_,n=k[a].leaf_,e=n.iTris_,p=g[b],q=g[c],r=g[d],s=this.verticesMap_,t=[Math.min(b,c),Math.max(b,c)],v=!1,w=s[t];void 0===w&&(w=g.length,v=!0,s[t]=w);r.ringVertices_.push(w);var u=3*a;m[u]=b;m[u+1]=w;m[u+2]=d;s=k.length;t=new Triangle(s);u=3*s;m[u]=w;m[u+1]=c;m[u+2]=d;t.stateFlag_=Mesh.stateMask_;r.tIndices_.push(s);q.replaceTriangle(a,s);t.leaf_=
n;t.posInLeaf_=e.length;if(v){var x=g.length,m=new Vertex(x),u=3*b,n=f[u],r=f[u+1],v=f[u+2],y=h[u],A=h[u+1],z=h[u+2],F=l[u],C=l[u+1],E=l[u+2],u=3*c,D=f[u],B=f[u+1],G=f[u+2],I=h[u],J=h[u+1],K=h[u+2],O=l[u],P=l[u+1],Q=l[u+2],L=y+I,M=A+J,N=z+K,H=1/Math.sqrt(L*L+M*M+N*N),u=3*x;h[u]=L*H;h[u+1]=M*H;h[u+2]=N*H;l[u]=0.5*(F+O);l[u+1]=0.5*(C+P);l[u+2]=0.5*(E+Q);l=0;this.linearSubdivision_||(H=y*I+A*J+z*K,l=0,l=-1>=H?Math.PI:1<=H?0:Math.acos(H),x=n-D,F=r-B,C=v-G,H=Math.sqrt(x*x+F*F+C*C),l=0.12*l*H,0>x*(y-I)+
F*(A-J)+C*(z-K)&&(l=-l));f[u]=0.5*(n+D)+h[u]*l;f[u+1]=0.5*(r+B)+h[u+1]*l;f[u+2]=0.5*(v+G)+h[u+2]*l;m.stateFlag_=Mesh.stateMask_;m.ringVertices_.push(b,c,d);p.replaceRingVertex(c,w);q.replaceRingVertex(b,w);m.tIndices_.push(a,s);g.push(m)}else b=g[w],b.ringVertices_.push(d),b.tIndices_.push(a,s);e.push(s);k.push(t)};
Topology.prototype.fillTriangles=function(a){for(var b=this.mesh_,c=b.vertices_,d=b.triangles_,b=b.indexArray_,e=a.length,g=[],k=0;k<e;++k){var f=a[k],h=3*f,l=b[h],m=b[h+1],h=b[h+2],n=this.verticesMap_,p=n[[Math.min(l,m),Math.max(l,m)]],q=n[[Math.min(m,h),Math.max(m,h)]],n=n[[Math.min(l,h),Math.max(l,h)]],r=c[l].ringVertices_.length,s=c[m].ringVertices_.length,t=c[h].ringVertices_.length,v=0;p?v=q?n?r<s&&r<t?2:s<t?3:1:r<t?2:1:n&&s<t?3:1:q?v=n&&s<r?3:2:n&&(v=3);if(1===v)this.fillTriangle(f,l,m,h,p);
else if(2===v)this.fillTriangle(f,m,h,l,q);else if(3===v)this.fillTriangle(f,h,l,m,n);else continue;g.push(f,d.length-1)}return g};
Topology.prototype.fillTriangle=function(a,b,c,d,e){var g=this.mesh_,k=g.vertices_,f=g.triangles_,g=g.indexArray_,h=3*a;g[h]=b;g[h+1]=e;g[h+2]=d;b=f[a].leaf_;var l=b.iTris_,m=k[c],n=k[d],h=k[e];h.ringVertices_.push(d);n.ringVertices_.push(e);k=f.length;h.tIndices_.push(a,k);var p=new Triangle(k),h=3*k;g[h]=e;g[h+1]=c;g[h+2]=d;p.stateFlag_=Mesh.stateMask_;p.leaf_=b;p.posInLeaf_=l.length;n.tIndices_.push(k);m.replaceTriangle(a,k);l.push(k);f.push(p)};Topology.prototype.decimation=function(a,b){var c=0,d=this.radiusSquared_,e=Math.sqrt(d),g=this.mesh_,k=g.triangles_,f=g.vertexArray_,g=g.indexArray_;this.iVertsDecimated_=[];this.iTrisToDelete_=[];this.iVertsToDelete_=[];for(var h=this.center_,l=h[0],m=h[1],h=h[2],n=[0,0,0],p=0;p<a.length;++p){var q=a[p];if(!(0>k[q].tagFlag_)){var c=3*q,r=g[c],s=g[c+1],t=g[c+2],c=3*r,v=f[c],w=f[c+1],u=f[c+2],c=3*s,x=f[c],y=f[c+1],A=f[c+2],c=3*t,z=f[c],F=f[c+1],c=f[c+2],C=(v+x+z)/3-l,E=(w+y+F)/3-m,D=(u+A+c)/3-h,B=
C*C+E*E+D*D;if(this.checkPlane_){var B=this.planeOrigin_,G=this.planeNormal_;if(0>vec3.dot(G,vec3.sub(n,[v,w,u],B))&&0>vec3.dot(G,vec3.sub(n,[x,y,A],B))&&0>vec3.dot(G,vec3.sub(n,[z,F,c],B)))continue;B=1}else if(B<d)B=1;else if(B<2*d)B=(Math.sqrt(B)-e)/(e*Math.SQRT2-e),B=3*B*B*B*B-4*B*B*B+1;else continue;C=x-v;E=y-w;D=A-u;G=C*C+E*E+D*D;C=x-z;E=y-F;D=A-c;x=C*C+E*E+D*D;C=v-z;E=w-F;D=u-c;v=C*C+E*E+D*D;G<x&&G<v?G<b*B&&this.decimateTriangles(q,this.findOppositeTriangle(q,r,s),a):x<v?x<b*B&&this.decimateTriangles(q,
this.findOppositeTriangle(q,s,t),a):v<b*B&&this.decimateTriangles(q,this.findOppositeTriangle(q,r,t),a)}}this.applyDeletion();return this.getValidModifiedTriangles(this.getValidModifiedVertices(),a)};Topology.prototype.applyDeletion=function(a){var b=this.iTrisToDelete_;Utils.tidy(b);for(var c=0,c=b.length-1;0<=c;--c)this.deleteTriangle(b[c],a);a=this.iVertsToDelete_;Utils.tidy(a);for(c=a.length-1;0<=c;--c)this.deleteVertex(a[c])};
...
Topology.prototype.edgeCollapse=function(a,b,c,d,e,g,k){var f=this.mesh_,h=f.vertices_,l=f.triangles_,m=f.vertexArray_,n=f.normalArray_,p=f.colorArray_,q=f.indexArray_,r=h[c],s=h[d],t=r.ringVertices_,v=s.ringVertices_,w=r.tIndices_,u=s.tIndices_,x=h[e],y=h[g];if(t.length===w.length&&v.length===u.length&&x.ringVertices_.length===x.tIndices_.length&&y.ringVertices_.length===y.tIndices_.length)if(this.iVertsDecimated_.push(c,d),this.states_.pushState(w,t),this.states_.pushState(u,v),t.sort(function(a,
b){return a-b}),v.sort(function(a,b){return a-b}),h=0,3<=Utils.intersectionArrays(t,v).length)r.removeTriangle(b),s.removeTriangle(a),x.tIndices_.push(b),y.tIndices_.push(a),h=3*a,q[h]===d?q[h]=g:q[h+1]===d?q[h+1]=g:q[h+2]=g,h=3*b,q[h]===c?q[h]=e:q[h+1]===c?q[h+1]=e:q[h+2]=e,f.computeRingVertices(c),f.computeRingVertices(d),f.computeRingVertices(e),f.computeRingVertices(g),this.cleanUpSingularVertex(c),this.cleanUpSingularVertex(d),this.cleanUpSingularVertex(e),this.cleanUpSingularVertex(g);else{var h=
3*c,A=3*d;e=n[h]+n[A];g=n[h+1]+n[A+1];var z=n[h+2]+n[A+2];p[h]=0.5*(p[h]+p[A]);p[h+1]=0.5*(p[h+1]+p[A+1]);p[h+2]=0.5*(p[h+2]+p[A+2]);p=1/Math.sqrt(e*e+g*g+z*z);e*=p;g*=p;z*=p;n[h]=e;n[h+1]=g;n[h+2]=z;t.push.apply(t,v);r.removeTriangle(a);r.removeTriangle(b);s.removeTriangle(a);s.removeTriangle(b);x.removeTriangle(a);y.removeTriangle(b);w.push.apply(w,u);r=u.length;for(n=n=0;n<r;++n)v=3*u[n],q[v]===d?q[v]=c:q[v+1]===d?q[v+1]=c:q[v+2]=c;f.computeRingVertices(c);r=u=q=0;v=t.length;for(n=0;n<v;++n)x=
t[n],f.computeRingVertices(x),x*=3,q+=m[x],u+=m[x+1],r+=m[x+2];q/=v;u/=v;r/=v;f=e*(q-m[h])+g*(u-m[h+1])+z*(r-m[h+2]);m[h]=q-e*f;m[h+1]=u-g*f;m[h+2]=r-z*f;s.tagFlag_=-1;l[a].tagFlag_=-1;l[b].tagFlag_=-1;this.iVertsToDelete_.push(d);this.iTrisToDelete_.push(a,b);k.push.apply(k,w);this.cleanUpSingularVertex(c)}};
...
Topology.prototype.deleteVertex=function(a){var b=this.mesh_,c=b.vertices_,d=b.vertexArray_,e=b.normalArray_,g=b.colorArray_,k=b.indexArray_,b=c.length-1;if(a!==b){var f=0,h=c[b],f=3*b,l=d[f],m=d[f+1],n=d[f+2],p=e[f],q=e[f+1],r=e[f+2],s=g[f],t=g[f+1],v=g[f+2],w=this.states_;w.pushState(null,[b]);h.id_=a;for(var u=h.tIndices_,x=h.ringVertices_,y=u.length,A=x.length,z=0,z=0;z<y;++z)f=u[z],w.pushState([f],null),f*=3,k[f]===b?k[f]=a:k[f+1]===b?k[f+1]=a:k[f+2]=a;for(z=0;z<A;++z)f=x[z],k=c[f],w.pushState(null,
[f]),k.replaceRingVertex(b,a);c[a]=h;f=3*a;d[f]=l;d[f+1]=m;d[f+2]=n;e[f]=p;e[f+1]=q;e[f+2]=r;g[f]=s;g[f+1]=t;g[f+2]=v}c.pop()};Topology.prototype.adaptTopology=function(a,b){for(var c=this.mesh_,d=c.triangles_,e=c.vertexArray_,g=this.radiusSquared_,k=c.getVerticesFromTriangles(a),f=k.length,h=[],l=c.vertices_.length,m=0,n=0,m=this.center_,p=m[0],q=m[1],r=m[2],s=0,t=0,m=n=0;m<f;++m){var v=k[m];v>=l||(n=3*v,s=e[n]-p,t=e[n+1]-q,n=e[n+2]-r,s*s+t*t+n*n<g&&h.push(v))}this.checkCollisions(h,b);m=c.getTrianglesFromVertices(this.iVertsDecimated_);a.push.apply(a,m);c=[];e=a.length;g=d.length;++Triangle.tagMask_;k=Triangle.tagMask_;
...
++Vertex.tagMask_;for(m=0;m<p;++m)d[g[m]].tagFlag_=Vertex.tagMask_;for(var g=3*f,q=e[g],r=e[g+1],s=e[g+2],t=n.getNeighborhood(q,r,s),v=t.length,m=0;m<v;++m){var w=t[m];if(f!==w){var u=d[w];if(!(0>u.tagFlag_||u.tagFlag_===Vertex.tagMask_)){var g=3*w,x=q-e[g],y=r-e[g+1],g=s-e[g+2];if(x*x+y*y+g*g<k){p>u.ringVertices_.length?this.vertexJoin(f,w):this.vertexJoin(w,f);break}}}}}this.applyDeletion();k=this.iVertsDecimated_=this.getValidModifiedVertices();h=k.length;e=[];n=Vertex.sculptMask_;for(l=0;l<h;++l)g=
...
Topology.prototype.cleanUpSingularVertex=function(a){var b=this.mesh_,c=b.vertices_,d=b.vertexArray_,e=b.normalArray_,g=b.colorArray_,k=b.indexArray_,f=c[a];if(!(0>f.tagFlag_)){var h=0,h=3*a,l=d[h],m=d[h+1],n=d[h+2],p=e[h],q=e[h+1],r=e[h+2],s=g[h],t=g[h+1],v=g[h+2];this.states_.pushState(f.tIndices_,f.ringVertices_);this.states_.pushState(null,[a]);if(!this.deleteVertexIfDegenerate(a)){var w=[];this.trianglesRotate(f.tIndices_,a,w);if(this.adjustEdgeOrientation(w)){for(var u=w.length,k=w[0].v1,x=
-1,d=0,d=1;d<u-1;++d)if(w[d].v2===k){x=d;break}if(-1!==x){this.checkArrayLength(1);var d=b.vertexArray_,e=b.normalArray_,g=b.colorArray_,k=b.indexArray_,u=c.length,y=new Vertex(u);y.stateFlag_=Mesh.stateMask_;h=3*u;d[h]=l;d[h+1]=m;d[h+2]=n;e[h]=-p;e[h+1]=-q;e[h+2]=-r;g[h]=s;g[h+1]=t;g[h+2]=v;for(d=0;d<=x;++d)l=w[d].t,y.tIndices_.push(l),f.removeTriangle(l),k[3*l+2]=u;c.push(y);b.computeRingVertices(a);b.computeRingVertices(u);c=y.ringVertices_;w=c.length;for(d=0;d<w;++d)b.computeRingVertices(c[d]);
f=f.ringVertices_;c=f.length;for(d=0;d<c;++d)b.computeRingVertices(f[d]);this.iVertsDecimated_.push(a,u);f=[];f.push(a,u);c=new Sculpt;c.mesh_=b;c.smooth(f,1);this.cleanUpSingularVertex(a);this.cleanUpSingularVertex(u)}}}}};
...
Topology.prototype.getProjectedVertex=function(a,b,c,d){var e=this.mesh_,g=e.vertexArray_,k=e.colorArray_,f=e.normalArray_,e=e.vertices_,h=[Math.min(a,b),Math.max(a,b)],l=this.verticesMap_[h];if(void 0===l){l=e.length;this.verticesMap_[h]=l;e.push(new Vertex(l));h=3*a;b*=3;var m=[0,0,0];Geometry.intersectLinePlane([g[h],g[h+1],g[h+2]],[g[b],g[b+1],g[b+2]],c,d,m);h=3*l;g[h]=m[0];g[h+1]=m[1];g[h+2]=m[2];f[h]=f[b];f[h+1]=f[b+1];f[h+2]=f[b+2];k[h]=k[b];k[h+1]=k[b+1];k[h+2]=k[b+2];e[a].ringVertices_.push(l);
e[l].ringVertices_.push(a)}return l};
Topology.prototype.triangulateVerticesOnPlane=function(a,b,c){for(var d=this.mesh_,e=d.vertexArray_,g=d.indexArray_,k=d.vertices_,f=[],h=d=0,l=a.length,d=0;d<l;++d)for(var m=a[d],n=k[m].tIndices_,p=n.length,h=0;h<p;++h){var q=3*n[h],r=g[q],s=g[q+1],q=g[q+2],t;r===m?(t=k[q],t.tIndices_.length!==t.ringVertices_.length&&f.push([m,q])):s===m?(t=k[r],t.tIndices_.length!==t.ringVertices_.length&&f.push([m,r])):(t=k[s],t.tIndices_.length!==t.ringVertices_.length&&f.push([m,s]))}a=[];this.detectHoles(f,a);
g=1;f=0;k=a.length;for(d=0;d<k;++d){l=a[d];m=l.length;for(h=1;h<m;++h)r=3*l[h-1],s=3*l[h],g+=vec3.sqrDist([e[r],e[r+1],e[r+2]],[e[s],e[s+1],e[s+2]]);f+=m-1}this.fillHoles(a,b,c);return g/f};
Topology.prototype.fillHoles=function(a,b,c){var d=this.mesh_,e=d.vertexArray_,g=d.indexArray_,k=d.vertices_,f=d.triangles_,h=0,l=0,m=b[0],n=b[1];b=b[2];var p=Geometry.getPerpendicularVector(c),q=[0,0,0];vec3.cross(q,p,c);for(var r=a.length,h=0;h<r;++h){var s=a[h],t=s.length,g=[];g.length=t;for(l=0;l<t;++l){var v=s[l],w=[e[3*v]-m,e[3*v+1]-n,e[3*v+2]-b],w=new poly2tri.Point(vec3.dot(p,w),vec3.dot(q,w));w.id_=v;g[l]=w}e=new poly2tri.SweepContext(g);e.triangulate();v=e.getTriangles();w=v.length;this.checkArrayLength(w/
100);e=d.vertexArray_;g=d.indexArray_;for(l=0;l<w;++l){var u=v[l].points_,x=f.length,y=new Triangle(x);y.normal_=c.slice();f.push(y);var y=u[0].id_,A=u[1].id_,u=u[2].id_;k[y].tIndices_.push(x);k[A].tIndices_.push(x);k[u].tIndices_.push(x);x*=3;g[x]=y;g[x+1]=u;g[x+2]=A}for(l=0;l<t;++l)d.computeRingVertices(s[l])}};
...
c=b.length,d=0;d<c;++d)if(a===b[d]){b[d]=b[c-1];b.pop();break}},removeRingVertex:function(a){for(var b=this.ringVertices_,c=b.length,d=0;d<c;++d)if(a===b[d]){b[d]=b[c-1];b.pop();break}}};function Gui(a){this.sculptgl_=a;this.foldTopo_=this.ctrlFillHoles_=this.ctrlCut_=this.ctrlFov_=this.ctrlNbTriangles_=this.ctrlNbVertices_=this.ctrlDetailDecimation_=this.ctrlDetailSubdivision_=this.ctrlCameraType_=this.ctrlIntensity_=this.ctrlRadius_=this.ctrlSculptCulling_=this.ctrlSymmetry_=this.ctrlContinuous_=this.ctrlNegative_=this.ctrlClay_=this.ctrlSculpt_=this.ctrlShaders_=this.ctrlColor_=null;this.open_=this.openFile;this.saveOBJ_=this.saveFileAsOBJ;this.savePLY_=this.saveFileAsPLY;this.saveSTL_=
this.saveFileAsSTL;this.keyVerold_="";this.exportVerold_=this.exportVerold;this.keySketchfab_="";this.exportSketchfab_=this.exportSketchfab;this.resetBg_=this.resetBackground;this.importBg_=this.importBackground;this.resetCamera_=this.resetCamera;this.dummyFunc_=function(){}}
Gui.prototype={initGui:function(){var a=new dat.GUI;a.domElement.style.position="absolute";a.domElement.style.height="650px";this.initGeneralGui(a);var b=new dat.GUI;this.initEditingGui(b);var c=this.sculptgl_;a.domElement.addEventListener("mouseout",function(){c.focusGui_=!1},!1);b.domElement.addEventListener("mouseout",function(){c.focusGui_=!1},!1);a.domElement.addEventListener("mouseover",function(){c.focusGui_=!0},!1);b.domElement.addEventListener("mouseover",function(){c.focusGui_=!0},!1)},
...
d.add(this,"exportVerold_").name("Upload");d=a.addFolder("Go to Sketchfab !");d.add(this,"keySketchfab_").name("API key");d.add(this,"exportSketchfab_").name("Upload");d=a.addFolder("Camera");d.add(this,"resetCamera_").name("Reset");d.add(b.camera_,"mode_",{Spherical:Camera.mode.SPHERICAL,Plane:Camera.mode.PLANE}).name("Mode").onChange(function(a){b.camera_.mode_=parseInt(a,10)});this.ctrlCameraType_=d.add(b.camera_,"type_",{Perspective:Camera.projType.PERSPECTIVE,Orthographic:Camera.projType.ORTHOGRAPHIC}).name("Type");
this.ctrlCameraType_.onChange(function(a){b.camera_.type_=parseInt(a,10);c.ctrlFov_.__li.hidden=b.camera_.type_===Camera.projType.ORTHOGRAPHIC;b.camera_.updateProjection();b.render()});this.ctrlFov_=d.add(b.camera_,"fov_",10,150).name("Fov");this.ctrlFov_.onChange(function(){b.camera_.updateProjection();b.render()});d.add(b.camera_,"usePivot_").name("Picking pivot").onChange(function(){b.camera_.toggleUsePivot();b.render()});d.open();d=a.addFolder("background");d.add(this,"resetBg_").name("Reset");
d.add(this,"importBg_").name("Import (jpg, png...)");d.open();a=a.addFolder("History");a.add(b,"undo_").name("Undo (Ctrl+Z)");a.add(b,"redo_").name("Redo (Ctrl+Y)");a.open()},initEditingGui:function(a){var b=this.sculptgl_,c=this,d=a.addFolder("Sculpt");this.ctrlSculpt_=d.add(b.sculpt_,"tool_",{"Brush (1)":Sculpt.tool.BRUSH,"Inflate (2)":Sculpt.tool.INFLATE,"Rotate (3)":Sculpt.tool.ROTATE,"Smooth (4)":Sculpt.tool.SMOOTH,"Flatten (5)":Sculpt.tool.FLATTEN,"Pinch (6)":Sculpt.tool.PINCH,"Crease (7)":Sculpt.tool.CREASE,
"Drag (8)":Sculpt.tool.DRAG,"Paint (9)":Sculpt.tool.COLOR,"Scale (0)":Sculpt.tool.SCALE,Cut:Sculpt.tool.CUT}).name("Tool");this.ctrlSculpt_.onChange(function(a){b.sculpt_.tool_=parseInt(a,10);a=b.sculpt_.tool_;var d=Sculpt.tool;c.ctrlClay_.__li.hidden=a!==d.BRUSH;c.ctrlNegative_.__li.hidden=a!==d.BRUSH&&a!==d.INFLATE&&a!==d.CREASE;c.ctrlContinuous_.__li.hidden=a===d.ROTATE||a===d.DRAG||a===d.SCALE||a===d.CUT;c.ctrlSymmetry_.__li.hidden=a===d.CUT;c.ctrlSculptCulling_.__li.hidden=a===d.CUT;c.ctrlRadius_.__li.hidden=
a===d.CUT;c.ctrlIntensity_.__li.hidden=c.ctrlContinuous_.__li.hidden;c.ctrlColor_.__li.hidden=a!==d.COLOR;c.ctrlCameraType_.__li.hidden=a===d.CUT;c.ctrlCut_.__li.hidden=a!==d.CUT;c.ctrlFillHoles_.__li.hidden=a!==d.CUT;c.foldTopo_.__ul.hidden=a===d.CUT;a===d.CUT?c.ctrlCameraType_.setValue(Camera.projType.ORTHOGRAPHIC):(b.sculpt_.lineOrigin_=[0,0],b.sculpt_.lineNormal_=[0,0])});this.ctrlClay_=d.add(b.sculpt_,"clay_").name("Clay");this.ctrlNegative_=d.add(b.sculpt_,"negative_").name("Negative (N)");
this.ctrlContinuous_=d.add(b,"continuous_").name("Continuous");this.ctrlSymmetry_=d.add(b,"symmetry_").name("Symmetry");this.ctrlSculptCulling_=d.add(b.sculpt_,"culling_").name("Sculpt culling");this.ctrlRadius_=d.add(b.picking_,"rDisplay_",5,200).name("Radius");this.ctrlIntensity_=d.add(b.sculpt_,"intensity_",0,1).name("Intensity");this.ctrlCut_=d.add(b,"cut_",0,1).name("Cut");this.ctrlCut_.__li.hidden=!0;this.ctrlFillHoles_=d.add(b.sculpt_,"fillHoles_",0,1).name("fill holes (buggy)");this.ctrlFillHoles_.__li.hidden=
!0;d.open();this.foldTopo_=a.addFolder("Topology");this.foldTopo_.add(b.sculpt_,"topo_",{Static:Sculpt.topo.STATIC,Dynamic:Sculpt.topo.SUBDIVISION,"Adaptive (!)":Sculpt.topo.ADAPTIVE}).name("Tool").onChange(function(a){b.sculpt_.topo_=parseInt(a,10);a=b.sculpt_.topo_;var d=Sculpt.topo;c.ctrlDetailSubdivision_.__li.hidden=a===d.STATIC;c.ctrlDetailDecimation_.__li.hidden=a!==d.SUBDIVISION});this.ctrlDetailSubdivision_=this.foldTopo_.add(b.sculpt_,"detailSubdivision_",0,1).name("Subdivision");this.ctrlDetailDecimation_=
this.foldTopo_.add(b.sculpt_,"detailDecimation_",0,1).name("Decimation");this.foldTopo_.open();a=a.addFolder("Mesh");this.ctrlNbVertices_=a.add(this,"dummyFunc_").name("Ver : 0");this.ctrlNbTriangles_=a.add(this,"dummyFunc_").name("Tri : 0");d={Phong:Render.mode.PHONG,Transparency:Render.mode.TRANSPARENCY,"Wireframe (slow)":Render.mode.WIREFRAME,"Normal shader":Render.mode.NORMAL,Clay:Render.mode.MATERIAL,Chavant:Render.mode.MATERIAL+1,Skin:Render.mode.MATERIAL+2,Drink:Render.mode.MATERIAL+3,"Red velvet":Render.mode.MATERIAL+
4,Orange:Render.mode.MATERIAL+5,Bronze:Render.mode.MATERIAL+6};this.ctrlShaders_=a.add(new Render,"shaderType_",d).name("Shader");this.ctrlShaders_.onChange(function(a){b.mesh_&&(b.mesh_.render_.updateShaders(parseInt(a,10),b.textures_,b.shaders_),b.mesh_.updateBuffers(),b.render())});this.ctrlColor_=a.addColor(b.sculpt_,"color_").name("Color");this.ctrlColor_.onChange(function(a){3===a.length?b.sculpt_.color_=[a[0],a[1],a[2]]:7===a.length?(a=parseInt(a.slice(1),16),b.sculpt_.color_=[a>>16,a>>8&255,
a&255]):b.sculpt_.color_=[168,66,66]});a.open()},updateMesh:function(a){a&&(this.ctrlShaders_.object=a.render_,this.ctrlShaders_.updateDisplay(),this.updateMeshInfo(a.vertices_.length,a.triangles_.length))},updateMeshInfo:function(a,b){this.ctrlNbVertices_.name("Ver : "+a);this.ctrlNbTriangles_.name("Tri : "+b)},openFile:function(){$("#fileopen").trigger("click")},resetBackground:function(){var a=this.sculptgl_.background_;a&&(a.gl_.deleteTexture(a.backgroundLoc_),this.sculptgl_.background_=null)},
resetCamera:function(){this.sculptgl_.camera_.reset();this.sculptgl_.render()},importBackground:function(){$("#backgroundopen").trigger("click")},saveFileAsOBJ:function(){if(this.sculptgl_.mesh_){var a=[Export.exportOBJ(this.sculptgl_.mesh_)],a=new Blob(a,{type:"text/plain;charset=utf-8"});saveAs(a,"yourMesh.obj")}},saveFileAsPLY:function(){if(this.sculptgl_.mesh_){var a=[Export.exportPLY(this.sculptgl_.mesh_)],a=new Blob(a,{type:"text/plain;charset=utf-8"});saveAs(a,"yourMesh.ply")}},saveFileAsSTL:function(){if(this.sculptgl_.mesh_){var a=
...
this.mesh_=this.background_=null;this.textures_={};this.shaders_={};this.sphere_="";this.gui_=new Gui(this);this.focusGui_=!1;this.resetSphere_=this.resetSphere;this.undo_=this.onUndo;this.redo_=this.onRedo;this.cut_=this.cut}SculptGL.elementIndexType=0;SculptGL.indexArrayType=Uint16Array;
...
2);b("ressources/drink.jpg",Render.mode.MATERIAL+3);b("ressources/redvelvet.jpg",Render.mode.MATERIAL+4);b("ressources/orange.jpg",Render.mode.MATERIAL+5);b("ressources/bronze.jpg",Render.mode.MATERIAL+6)},loadShaders:function(){var a=function(a){var b=new XMLHttpRequest;b.open("GET",a,!1);b.send(null);return b.responseText},b=this.shaders_;b.phongVertex=a("shaders/phong.vert");b.phongFragment=a("shaders/phong.frag");b.transparencyVertex=a("shaders/transparency.vert");b.transparencyFragment=a("shaders/transparency.frag");
b.wireframeVertex=a("shaders/wireframe.vert");b.wireframeFragment=a("shaders/wireframe.frag");b.normalVertex=a("shaders/normal.vert");b.normalFragment=a("shaders/normal.frag");b.reflectionVertex=a("shaders/reflection.vert");b.reflectionFragment=a("shaders/reflection.frag");b.backgroundVertex=a("shaders/background.vert");b.backgroundFragment=a("shaders/background.frag")},loadSphere:function(){var a=new XMLHttpRequest;a.open("GET","ressources/sphere.obj",!0);var b=this;a.onload=function(){b.sphere_=
this.responseText;b.resetSphere()};a.send(null)},render:function(){var a=this.gl_;a.clear(a.COLOR_BUFFER_BIT|a.DEPTH_BUFFER_BIT);this.camera_.updateView();this.background_&&(a.depthMask(!1),this.background_.render(),a.depthMask(!0));this.mesh_&&this.mesh_.render(this.camera_,this.picking_,this.sculpt_.lineOrigin_,this.sculpt_.lineNormal_)},onWindowResize:function(){var a=$(window).width(),b=$(window).height();this.camera_.width_=a;this.camera_.height_=b;$("#canvas").attr("width",a);$("#canvas").attr("height",
