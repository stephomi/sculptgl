...
h=h.split(/\s+/),s=parseInt(h[0],10);if(3===s||4===s){var n=parseInt(h[1],10),t=parseInt(h[2],10),q=parseInt(h[3],10),l=c[n],v=c[t],r=c[q],w=d.length;l.tIndices_.push(w);v.tIndices_.push(w);r.tIndices_.push(w);d.push(new Triangle(w));k.push(n,t,q);4===s&&(++w,h=parseInt(h[4],10),s=c[h],l.tIndices_.push(w),r.tIndices_.push(w),s.tIndices_.push(w),d.push(new Triangle(w)),k.push(n,q,h))}}break}++p}Import.initMeshArrays(b,e,k,g)},importSTL:function(a,b){84+50*Import.getUint32(a,80)===a.length?Import.importBinarySTL(a,
...
exportSketchfab:function(a,b){var c=new FormData;c.append("token",b);var d=Export.exportOBJ(a);c.append("fileModel",new Blob([d]));c.append("filenameModel","model.obj");var e=new XMLHttpRequest;e.open("POST","https://api.sketchfab.com/v1/models");e.addEventListener("load",function(){var a=JSON.parse(e.responseText);console.log(a);a.success?alert("Upload success !"):alert("Sketchfab upload error :\n"+a.error)},!0);e.send(c)}};var Geometry={normalizedMouse:function(a,b,c,d){return[2*a/c-1,1-2*b/d]},mouseOnUnitSphere:function(a){var b=a[0];a=a[1];var c=1-b*b-a*a,c=0<c?Math.sqrt(c):0,b=[b,a,c];return vec3.normalize(b,b)}};
Geometry.intersectionRayTriangle=function(){var a=[0,0,0],b=[0,0,0],c=[0,0,0],d=[0,0,0];return function(e,g,k,f,h,l){vec3.sub(a,f,k);vec3.sub(b,h,k);vec3.cross(c,g,b);f=vec3.dot(a,c);if(-1E-6<f&&1E-6>f)return!1;f=1/f;vec3.sub(d,e,k);k=vec3.dot(d,c)*f;if(0>k||1<k)return!1;h=[0,0,0];vec3.cross(h,d,a);var m=vec3.dot(g,h)*f;if(0>m||1<k+m)return!1;k=vec3.dot(b,h)*f;if(0>k)return!1;vec3.scaleAndAdd(l,e,g,k);return!0}}();
Geometry.computeTriangleAabb=function(a,b,c,d,e,g,k,f,h,l){var m=a.min_;a=a.max_;m[0]=Math.min(b,e,f);m[1]=Math.min(c,g,h);m[2]=Math.min(d,k,l);a[0]=Math.max(b,e,f);a[1]=Math.max(c,g,h);a[2]=Math.max(d,k,l)};Geometry.triangleNormal=function(a,b,c,d,e,g,k,f,h,l){e-=b;g-=c;k-=d;b=f-b;c=h-c;l-=d;d=g*l-k*c;l=k*b-e*l;e=e*c-g*b;g=d*d+l*l+e*e;0!==g&&(g=1/Math.sqrt(g),a[0]=d*g,a[1]=l*g,a[2]=e*g)};
...
100);e=g.length;f=k.length;this.cleanCutTriangles(l,h,a,b);g=g.length;k=k.length;this.iVertsToDelete_=m;this.iTrisToDelete_=h;for(h=[];f<k;++f)h.push(f);d.updateTrianglesAabbAndNormal(h);h=[];for(f=e;f<g;++f)h.push(f);d.updateVerticesNormal(h);f=0;c&&(f=this.triangulateVerticesOnPlane(h,a,b));this.applyDeletion(!0);this.states_.recomputeOctree(d.octree_.aabbSplit_);return f};
...
this.mesh_;this.sculpt_.mesh_=this.mesh_;Mesh.stateMask_=1;Vertex.tagMask_=1;Vertex.sculptMask_=1;Triangle.tagMask_=1},endMeshLoad:function(){var a=this.mesh_;a.initMesh();a.moveTo([0,0,0]);this.camera_.reset();this.gui_.updateMesh(a);a.initRender(Shader.mode.MATERIAL,this.textures_,this.shaders_);this.render()},onUndo:function(){this.states_.undo();this.render();this.gui_.updateMesh(this.mesh_)},onRedo:function(){this.states_.redo();this.render();this.gui_.updateMesh(this.mesh_)}};
