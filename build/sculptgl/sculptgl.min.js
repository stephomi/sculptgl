...
g,d,h,f){var k=this.rotateData_;f&&(k=this.rotateDataSym_);f=k.center2d;g=[e-f[0],g-f[1]];if(!(30>vec2.len(g))){vec2.normalize(g,g);k=k.normal;e=[0,0,0,0];d=[d-f[0],h-f[1]];vec2.normalize(d,d);d=Geometry.signedAngle2d(g,d);h=this.mesh_.vertexArray_;c=Math.sqrt(c);f=b.length;g=a[0];for(var l=a[1],m=a[2],n=0;n<f;++n){var p=3*b[n],q=h[p]-g,r=h[p+1]-l,s=h[p+2]-m,q=Math.sqrt(q*q+r*r+s*s)/c,r=[h[p],h[p+1],h[p+2]];quat.setAxisAngle(e,k,d*(3*q*q*q*q-4*q*q*q+1));vec3.sub(r,r,a);vec3.transformQuat(r,r,e);vec3.add(r,
r,a);h[p]=r[0];h[p+1]=r[1];h[p+2]=r[2]}}},smooth:function(a,b){var c=this.mesh_.vertexArray_,e=a.length,g=new Float32Array(3*e);this.laplacianSmooth(a,g);for(var d=this.d2Move_,h=Math.sqrt(d),f=this.topo_===Sculpt.topo.ADAPTIVE,k=0;k<e;++k){var l=3*a[k],m=3*k,n=(g[m]-c[l])*b,p=(g[m+1]-c[l+1])*b,m=(g[m+2]-c[l+2])*b;if(f){var q=n*n+p*p+m*m;q>d&&(q=Math.sqrt(q),n=n/q*h,p=p/q*h,m=m/q*h)}c[l]+=n;c[l+1]+=p;c[l+2]+=m}},flatten:function(a,b,c,e,g){var d=this.areaNormal(c);if(d){var h=this.areaCenter(c);c=
this.mesh_.vertexArray_;e=Math.sqrt(e);var f=b.length;g*=0.3;var k=a[0],l=a[1];a=a[2];for(var m=h[0],n=h[1],h=h[2],p=d[0],q=d[1],d=d[2],r=Math.sqrt(this.d2Move_),s=this.topo_===Sculpt.topo.ADAPTIVE,u=0;u<f;++u){var v=3*b[u],t=c[v],w=c[v+1],x=c[v+2],y=(t-m)*p+(w-n)*q+(x-h)*d,t=t-k,w=w-l,x=x-a,x=Math.sqrt(t*t+w*w+x*x)/e,x=3*x*x*x*x-4*x*x*x+1,x=s?Math.min(r,y*g*x):y*g*x;c[v]-=p*x;c[v+1]-=q*x;c[v+2]-=d*x}}},smoothFlat:function(a,b){var c=this.mesh_,e=c.vertexArray_,c=c.normalArray_,g=a.length,d=new Float32Array(3*
