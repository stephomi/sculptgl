...
0,b[h+6]=0,b[h+7]=0,b[h+8]=1;this.barycenterBuffer_||(this.barycenterBuffer_=e.createBuffer());e.bindBuffer(e.ARRAY_BUFFER,this.barycenterBuffer_);e.bufferData(e.ARRAY_BUFFER,b,e.DYNAMIC_DRAW)}};function Sculpt(a){this.states_=a;this.mesh_=null;this.intensity_=0.75;this.tool_=Sculpt.tool.BRUSH;this.topo_=Sculpt.topo.SUBDIVISION;this.detail_=0.75;this.culling_=this.negative_=!1;this.d2Max_=this.d2Min_=0;this.d2Thickness_=0.5;this.d2Move_=0;this.rotateData_={normal:[0,0,0],center2d:[0,0]}}Sculpt.tool={BRUSH:0,INFLATE:1,ROTATE:2,SMOOTH:3,FLATTEN:4};Sculpt.topo={STATIC:0,SUBDIVISION:1,DECIMATION:2,UNIFORMISATION:3,ADAPTIVE:4};
Sculpt.prototype={setAdaptiveParameters:function(a){this.d2Max_=0.2*a*(1.1-this.detail_);this.d2Min_=this.d2Max_/4.2025;this.d2Move_=0.2375*this.d2Min_;this.d2Thickness_=1.1*(4*this.d2Move_+this.d2Max_/3)},sculptMesh:function(a,b,c,e,g){var d=this.mesh_,h=a.pickedVertices_,f=a.rWorldSqr_,k=a.interPoint_,l=a.eyeDir_;a=d.vertices_;var m=d.getTrianglesFromVertices(h);this.states_.pushState(m,h);var n=new Topology(this.states_);n.mesh_=d;n.radiusSquared_=f;n.center_=k;this.setAdaptiveParameters(f);switch(this.topo_){case Sculpt.topo.SUBDIVISION:m=
n.subdivision(m,this.d2Max_);break;case Sculpt.topo.DECIMATION:m=n.decimation(m,this.d2Min_);break;case Sculpt.topo.UNIFORMISATION:case Sculpt.topo.ADAPTIVE:m=n.subdivision(m,this.d2Max_),m=n.decimation(m,this.d2Min_)}for(var h=d.getVerticesFromTriangles(m),p=h.length,q=[],r=[],s=Vertex.sculptMask_,u=d.normalArray_,v=l[0],t=l[1],l=l[2],w=0;w<p;++w){var x=h[w];if(a[x].sculptFlag_===s){q.push(x);var y=3*x;0>=u[y]*v+u[y+1]*t+u[y+2]*l&&r.push(x)}}this.culling_&&(q=r);switch(this.tool_){case Sculpt.tool.BRUSH:this.flatten(k,
q,r,f,0.5*this.intensity_);this.brush(k,q,r,f,this.intensity_);break;case Sculpt.tool.INFLATE:this.inflate(k,q,f,this.intensity_);break;case Sculpt.tool.ROTATE:this.rotate(k,q,f,b,c,e,g);break;case Sculpt.tool.SMOOTH:this.smooth(q,this.intensity_);break;case Sculpt.tool.FLATTEN:this.flatten(k,q,r,f,this.intensity_)}this.topo_===Sculpt.topo.ADAPTIVE&&(m=n.adaptTopology(m,this.d2Thickness_),h=d.getVerticesFromTriangles(m));d.updateMesh(m,h)},brush:function(a,b,c,e,g){var d=this.areaNormal(c);c=this.mesh_.vertexArray_;
e=Math.sqrt(e);var h=b.length;g=0.1*g*e;this.topo_===Sculpt.topo.ADAPTIVE&&(g=Math.min(Math.sqrt(this.d2Move_),g));this.negative_&&(g=-g);var f=a[0],k=a[1];a=a[2];for(var l=d[0],m=d[1],d=d[2],n=0;n<h;++n){var p=3*b[n],q=c[p]-f,r=c[p+1]-k,s=c[p+2]-a,q=Math.sqrt(q*q+r*r+s*s)/e,q=3*q*q*q*q-4*q*q*q+1;c[p]+=l*g*q;c[p+1]+=m*g*q;c[p+2]+=d*g*q}},inflate:function(a,b,c,e){var g=this.mesh_,d=g.vertexArray_,g=g.normalArray_,h=b.length;c=Math.sqrt(c);e=0.1*e*c;this.topo_===Sculpt.topo.ADAPTIVE&&(e=Math.min(Math.sqrt(this.d2Move_),
e));this.negative_&&(e=-e);var f=a[0],k=a[1];a=a[2];for(var l=0;l<h;++l){var m=3*b[l],n=d[m]-f,p=d[m+1]-k,q=d[m+2]-a,n=Math.sqrt(n*n+p*p+q*q)/c,n=3*n*n*n*n-4*n*n*n+1;d[m]+=g[m]*e*n;d[m+1]+=g[m+1]*e*n;d[m+2]+=g[m+2]*e*n}},startRotate:function(a,b,c){var e=this.rotateData_,g=this.mesh_,d=Geometry.point2Dto3D(a.camera_,b,c,0),h=Geometry.point2Dto3D(a.camera_,b,c,1),f=mat4.create();mat4.invert(f,g.matTransform_);vec3.transformMat4(d,d,f);vec3.transformMat4(h,h,f);a.intersectionRayMesh(g,d,h,b,c);a.mesh_&&
(a.pickVerticesInSphere(a.rWorldSqr_),a=[0,0,0],vec3.sub(a,d,h),vec3.normalize(a,a),e.normal=a,e.center2d=[b,c])},rotate:function(a,b,c,e,g,d,h){var f=this.rotateData_,k=f.center2d;g=[e-k[0],g-k[1]];if(!(30>vec2.len(g))){vec2.normalize(g,g);f=f.normal;e=[0,0,0,0];d=[d-k[0],h-k[1]];vec2.normalize(d,d);d=Geometry.signedAngle2d(g,d);h=this.mesh_.vertexArray_;c=Math.sqrt(c);k=b.length;g=a[0];for(var l=a[1],m=a[2],n=0;n<k;++n){var p=3*b[n],q=h[p]-g,r=h[p+1]-l,s=h[p+2]-m,q=Math.sqrt(q*q+r*r+s*s)/c,r=[h[p],
h[p+1],h[p+2]];quat.setAxisAngle(e,f,d*(3*q*q*q*q-4*q*q*q+1));vec3.sub(r,r,a);vec3.transformQuat(r,r,e);vec3.add(r,r,a);h[p]=r[0];h[p+1]=r[1];h[p+2]=r[2]}}},smooth:function(a,b){var c=this.mesh_.vertexArray_,e=a.length,g=new Float32Array(3*e);this.laplacianSmooth(a,g);for(var d=this.d2Move_,h=Math.sqrt(d),f=this.topo_===Sculpt.topo.ADAPTIVE,k=0;k<e;++k){var l=3*a[k],m=3*k,n=(g[m]-c[l])*b,p=(g[m+1]-c[l+1])*b,m=(g[m+2]-c[l+2])*b;if(f){var q=n*n+p*p+m*m;q>d&&(q=Math.sqrt(q),n=n/q*h,p=p/q*h,m=m/q*h)}c[l]+=
n;c[l+1]+=p;c[l+2]+=m}},flatten:function(a,b,c,e,g){var d=this.areaNormal(c),h=this.areaCenter(c);c=this.mesh_.vertexArray_;e=Math.sqrt(e);var f=b.length;g*=0.3;var k=a[0],l=a[1];a=a[2];for(var m=h[0],n=h[1],h=h[2],p=d[0],q=d[1],d=d[2],r=Math.sqrt(this.d2Move_),s=this.topo_===Sculpt.topo.ADAPTIVE,u=0;u<f;++u){var v=3*b[u],t=c[v],w=c[v+1],x=c[v+2],y=(t-m)*p+(w-n)*q+(x-h)*d,t=t-k,w=w-l,x=x-a,x=Math.sqrt(t*t+w*w+x*x)/e,x=3*x*x*x*x-4*x*x*x+1,x=s?Math.min(r,y*g*x):y*g*x;c[v]-=p*x;c[v+1]-=q*x;c[v+2]-=d*
x}},smoothFlat:function(a,b){var c=this.mesh_,e=c.vertexArray_,c=c.normalArray_,g=a.length,d=new Float32Array(3*g);this.laplacianSmooth(a,d);for(var h=0;h<g;++h){var f=3*a[h],k=e[f],l=e[f+1],m=e[f+2],n=c[f],p=c[f+1],q=c[f+2],r=3*h,s=d[r],u=d[r+1],r=d[r+2],v=n*(s-k)+p*(u-l)+q*(r-m);e[f]+=(s-n*v-k)*b;e[f+1]+=(u-p*v-l)*b;e[f+2]+=(r-q*v-m)*b}},laplacianSmooth:function(a,b){for(var c=this.mesh_,e=c.vertices_,c=c.vertexArray_,g=a.length,d=0;d<g;++d){var h=3*d,f=e[a[d]],k=f.ringVertices_,l=k.length,m=0,
n=0,p=0,q=0,r=0;if(l!==f.tIndices_.length){for(q=f=0;q<l;++q){var r=k[q],s=e[r];s.ringVertices_.length!==s.tIndices_.length&&(r*=3,m+=c[r],n+=c[r+1],p+=c[r+2],++f)}b[h]=m/f;b[h+1]=n/f;b[h+2]=p/f}else{for(q=0;q<l;++q)r=3*k[q],m+=c[r],n+=c[r+1],p+=c[r+2];b[h]=m/l;b[h+1]=n/l;b[h+2]=p/l}}},areaNormal:function(a){for(var b=this.mesh_.normalArray_,c=a.length,e=0,g=0,d=0,h=0;h<c;++h)var f=3*a[h],e=e+b[f],g=g+b[f+1],d=d+b[f+2];a=1/Math.sqrt(e*e+g*g+d*d);return[e*a,g*a,d*a]},areaCenter:function(a){for(var b=
this.mesh_.vertexArray_,c=a.length,e=0,g=0,d=0,h=0;h<c;++h)var f=3*a[h],e=e+b[f],g=g+b[f+1],d=d+b[f+2];return[e/c,g/c,d/c]}};var Tools={nextHighestPowerOfTwo:function(a){--a;for(var b=1;32>b;b<<=1)a|=a>>b;return a+1},tidy:function(a){a.sort(function(a,b){return a-b});for(var b=a.length,c=0,e=0,c=1;c<b;++c)a[e]!==a[c]&&(a[++e]=a[c]);1<c&&(a.length=e+1)},intersectionArrays:function(a,b){for(var c=0,e=0,g=[],d=a.length,h=b.length;c<d&&e<h;)a[c]<b[e]?c++:a[c]>b[e]?e++:(g.push(a[c]),++c,++e);return g}};"function"!==typeof String.prototype.endsWith&&(String.prototype.endsWith=function(a){return this.slice(-a.length)===a});
...
this.ctrlSculpt_=c.add(this.sculpt_,"tool_",{"Brush (1)":Sculpt.tool.BRUSH,"Inflate (2)":Sculpt.tool.INFLATE,"Rotate (3)":Sculpt.tool.ROTATE,"Smooth (4)":Sculpt.tool.SMOOTH,"Flatten (5)":Sculpt.tool.FLATTEN}).name("Tool");this.ctrlSculpt_.onChange(function(a){b.sculpt_.tool_=parseInt(a,10)});this.ctrlNegative_=c.add(this.sculpt_,"negative_").name("Negative (N)");c.add(this.sculpt_,"culling_").name("Sculpt culling");c.add(this.picking_,"rDisplay_",20,200).name("Radius");c.add(this.sculpt_,"intensity_",
0,1).name("Intensity");c.open();a=a.addFolder("Topology");a.add(this.sculpt_,"topo_",{Static:Sculpt.topo.STATIC,Subdivision:Sculpt.topo.SUBDIVISION,Decimation:Sculpt.topo.DECIMATION,Uniformisation:Sculpt.topo.UNIFORMISATION,"Adaptive (!!!)":Sculpt.topo.ADAPTIVE}).name("Tool").onChange(function(a){b.sculpt_.topo_=parseInt(a,10)});a.add(this.sculpt_,"detail_",0,1).name("Detail");a.open()},render:function(){var a=this.gl_;a.clear(a.COLOR_BUFFER_BIT|a.DEPTH_BUFFER_BIT);this.camera_.updateView();this.mesh_&&
