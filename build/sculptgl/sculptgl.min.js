...
switch(this.topo_){case Sculpt.topo.SUBDIVISION:n=p.subdivision(n,this.d2Max_);break;case Sculpt.topo.DECIMATION:n=p.decimation(n,this.d2Min_);break;case Sculpt.topo.UNIFORMISATION:case Sculpt.topo.ADAPTIVE:n=p.subdivision(n,this.d2Max_),n=p.decimation(n,this.d2Min_)}for(var f=h.getVerticesFromTriangles(n),q=f.length,r=[],s=[],u=Vertex.sculptMask_,v=h.normalArray_,t=m[0],w=m[1],m=m[2],x=0;x<q;++x){var y=f[x];if(a[y].sculptFlag_===u){r.push(y);var z=3*y;0>=v[z]*t+v[z+1]*w+v[z+2]*m&&s.push(y)}}this.culling_&&
(r=s);switch(this.tool_){case Sculpt.tool.BRUSH:this.flatten(l,r,s,k,0.5*this.intensity_);this.brush(l,r,s,k,this.intensity_);break;case Sculpt.tool.INFLATE:this.inflate(l,r,k,this.intensity_);break;case Sculpt.tool.ROTATE:this.rotate(l,r,k,b,c,e,g,d);break;case Sculpt.tool.SMOOTH:this.smooth(r,this.intensity_);break;case Sculpt.tool.FLATTEN:this.flatten(l,r,s,k,this.intensity_)}this.topo_===Sculpt.topo.ADAPTIVE&&(n=p.adaptTopology(n,this.d2Thickness_),f=h.getVerticesFromTriangles(n));h.updateMesh(n,
f)},brush:function(a,b,c,e,g){var d=this.areaNormal(c);c=this.mesh_.vertexArray_;e=Math.sqrt(e);var h=b.length;g=0.1*g*e;this.topo_===Sculpt.topo.ADAPTIVE&&(g=Math.min(Math.sqrt(this.d2Move_),g));this.negative_&&(g=-g);var f=a[0],k=a[1];a=a[2];for(var l=d[0],m=d[1],d=d[2],n=0;n<h;++n){var p=3*b[n],q=c[p]-f,r=c[p+1]-k,s=c[p+2]-a,q=Math.sqrt(q*q+r*r+s*s)/e,q=3*q*q*q*q-4*q*q*q+1;c[p]+=l*g*q;c[p+1]+=m*g*q;c[p+2]+=d*g*q}},inflate:function(a,b,c,e){var g=this.mesh_,d=g.vertexArray_,g=g.normalArray_,h=b.length;
c=Math.sqrt(c);e=0.1*e*c;this.topo_===Sculpt.topo.ADAPTIVE&&(e=Math.min(Math.sqrt(this.d2Move_),e));this.negative_&&(e=-e);var f=a[0],k=a[1];a=a[2];for(var l=0;l<h;++l){var m=3*b[l],n=d[m]-f,p=d[m+1]-k,q=d[m+2]-a,n=Math.sqrt(n*n+p*p+q*q)/c,n=3*n*n*n*n-4*n*n*n+1;d[m]+=g[m]*e*n;d[m+1]+=g[m+1]*e*n;d[m+2]+=g[m+2]*e*n}},startRotate:function(a,b,c,e,g,d){var h=Geometry.point2Dto3D(a.camera_,b,c,0),f=Geometry.point2Dto3D(a.camera_,b,c,1),k=mat4.create();mat4.invert(k,this.mesh_.matTransform_);vec3.transformMat4(h,
h,k);vec3.transformMat4(f,f,k);this.initRotateData(a,h,f,b,c,this.rotateData_);e&&(h=[h[0],h[1],h[2]],Geometry.mirrorPoint(h,g,d),f=[f[0],f[1],f[2]],Geometry.mirrorPoint(f,g,d),this.initRotateData(e,h,f,b,c,this.rotateDataSym_),e.rWorldSqr_=a.rWorldSqr_)},initRotateData:function(a,b,c,e,g,d){a.intersectionRayMesh(this.mesh_,b,c,e,g);a.mesh_&&(a.pickVerticesInSphere(a.rWorldSqr_),a=[0,0,0],vec3.sub(a,b,c),d.normal=vec3.normalize(a,a),d.center2d=[e,g])},rotate:function(a,b,c,e,g,d,h,f){var k=this.rotateData_;
f&&(k=this.rotateDataSym_);f=k.center2d;g=[e-f[0],g-f[1]];if(!(30>vec2.len(g))){vec2.normalize(g,g);k=k.normal;e=[0,0,0,0];d=[d-f[0],h-f[1]];vec2.normalize(d,d);d=Geometry.signedAngle2d(g,d);h=this.mesh_.vertexArray_;c=Math.sqrt(c);f=b.length;g=a[0];for(var l=a[1],m=a[2],n=0;n<f;++n){var p=3*b[n],q=h[p]-g,r=h[p+1]-l,s=h[p+2]-m,q=Math.sqrt(q*q+r*r+s*s)/c,r=[h[p],h[p+1],h[p+2]];quat.setAxisAngle(e,k,d*(3*q*q*q*q-4*q*q*q+1));vec3.sub(r,r,a);vec3.transformQuat(r,r,e);vec3.add(r,r,a);h[p]=r[0];h[p+1]=
r[1];h[p+2]=r[2]}}},smooth:function(a,b){var c=this.mesh_.vertexArray_,e=a.length,g=new Float32Array(3*e);this.laplacianSmooth(a,g);for(var d=this.d2Move_,h=Math.sqrt(d),f=this.topo_===Sculpt.topo.ADAPTIVE,k=0;k<e;++k){var l=3*a[k],m=3*k,n=(g[m]-c[l])*b,p=(g[m+1]-c[l+1])*b,m=(g[m+2]-c[l+2])*b;if(f){var q=n*n+p*p+m*m;q>d&&(q=Math.sqrt(q),n=n/q*h,p=p/q*h,m=m/q*h)}c[l]+=n;c[l+1]+=p;c[l+2]+=m}},flatten:function(a,b,c,e,g){var d=this.areaNormal(c),h=this.areaCenter(c);c=this.mesh_.vertexArray_;e=Math.sqrt(e);
var f=b.length;g*=0.3;var k=a[0],l=a[1];a=a[2];for(var m=h[0],n=h[1],h=h[2],p=d[0],q=d[1],d=d[2],r=Math.sqrt(this.d2Move_),s=this.topo_===Sculpt.topo.ADAPTIVE,u=0;u<f;++u){var v=3*b[u],t=c[v],w=c[v+1],x=c[v+2],y=(t-m)*p+(w-n)*q+(x-h)*d,t=t-k,w=w-l,x=x-a,x=Math.sqrt(t*t+w*w+x*x)/e,x=3*x*x*x*x-4*x*x*x+1,x=s?Math.min(r,y*g*x):y*g*x;c[v]-=p*x;c[v+1]-=q*x;c[v+2]-=d*x}},smoothFlat:function(a,b){var c=this.mesh_,e=c.vertexArray_,c=c.normalArray_,g=a.length,d=new Float32Array(3*g);this.laplacianSmooth(a,
d);for(var h=0;h<g;++h){var f=3*a[h],k=e[f],l=e[f+1],m=e[f+2],n=c[f],p=c[f+1],q=c[f+2],r=3*h,s=d[r],u=d[r+1],r=d[r+2],v=n*(s-k)+p*(u-l)+q*(r-m);e[f]+=(s-n*v-k)*b;e[f+1]+=(u-p*v-l)*b;e[f+2]+=(r-q*v-m)*b}},laplacianSmooth:function(a,b){for(var c=this.mesh_,e=c.vertices_,c=c.vertexArray_,g=a.length,d=0;d<g;++d){var h=3*d,f=e[a[d]],k=f.ringVertices_,l=k.length,m=0,n=0,p=0,q=0,r=0;if(l!==f.tIndices_.length){for(q=f=0;q<l;++q){var r=k[q],s=e[r];s.ringVertices_.length!==s.tIndices_.length&&(r*=3,m+=c[r],
n+=c[r+1],p+=c[r+2],++f)}b[h]=m/f;b[h+1]=n/f;b[h+2]=p/f}else{for(q=0;q<l;++q)r=3*k[q],m+=c[r],n+=c[r+1],p+=c[r+2];b[h]=m/l;b[h+1]=n/l;b[h+2]=p/l}}},areaNormal:function(a){for(var b=this.mesh_.normalArray_,c=a.length,e=0,g=0,d=0,h=0;h<c;++h)var f=3*a[h],e=e+b[f],g=g+b[f+1],d=d+b[f+2];a=1/Math.sqrt(e*e+g*g+d*d);return[e*a,g*a,d*a]},areaCenter:function(a){for(var b=this.mesh_.vertexArray_,c=a.length,e=0,g=0,d=0,h=0;h<c;++h)var f=3*a[h],e=e+b[f],g=g+b[f+1],d=d+b[f+2];return[e/c,g/c,d/c]}};var Tools={nextHighestPowerOfTwo:function(a){--a;for(var b=1;32>b;b<<=1)a|=a>>b;return a+1},tidy:function(a){a.sort(function(a,b){return a-b});for(var b=a.length,c=0,e=0,c=1;c<b;++c)a[e]!==a[c]&&(a[++e]=a[c]);1<c&&(a.length=e+1)},intersectionArrays:function(a,b){for(var c=0,e=0,g=[],d=a.length,h=b.length;c<d&&e<h;)a[c]<b[e]?c++:a[c]>b[e]?e++:(g.push(a[c]),++c,++e);return g}};"function"!==typeof String.prototype.endsWith&&(String.prototype.endsWith=function(a){return this.slice(-a.length)===a});
