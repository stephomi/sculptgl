...
Picking.prototype={intersectionMouseMesh:function(a,b,c,e,g,d){var h=Geometry.point2Dto3D(this.camera_,b,c,0),f=Geometry.point2Dto3D(this.camera_,b,c,1),k=mat4.create();mat4.invert(k,a.matTransform_);vec3.transformMat4(h,h,k);vec3.transformMat4(f,f,k);g&&(Geometry.mirrorPoint(h,g,d),Geometry.mirrorPoint(f,g,d));this.intersectionRayMesh(a,h,f,b,c,e);a=this.eyeDir_;vec3.sub(a,f,h);vec3.normalize(a,a)},intersectionRayMesh:function(a,b,c,e,g,d){this.mesh_=null;this.pickedTriangle_=-1;var h=a.triangles_,
f=a.vertexArray_,k=a.indexArray_,l=[0,0,0];vec3.sub(l,c,b);vec3.normalize(l,l);for(var l=a.octree_.intersectRay(b,[1/l[0],1/l[1],1/l[2]]),m=-1,n=l.length,p=[0,0,0],q=0;q<n;++q){var s=l[q],r=h[s],s=3*s,t=3*k[s],v=3*k[s+1],s=3*k[s+2];if(Geometry.intersectionRayTriangle(b,c,[f[t],f[t+1],f[t+2]],[f[v],f[v+1],f[v+2]],[f[s],f[s+1],f[s+2]],r.normal_,p)&&(r=vec3.sqrDist(b,p),r<m||-1===m))m=r,vec3.copy(this.interPoint_,p),this.pickedTriangle_=l[q]}-1!==this.pickedTriangle_?(this.mesh_=a,a=[0,0,0],vec3.transformMat4(a,
this.interPoint_,this.mesh_.matTransform_),b=Geometry.point3Dto2D(this.camera_,a)[2],e=Geometry.point2Dto3D(this.camera_,e+this.rDisplay_*d,g,b),this.rWorldSqr_=vec3.sqrDist(a,e)):this.rWorldSqr_=0},pickVerticesInSphere:function(a){this.pickedVertices_=[];for(var b=this.mesh_.vertices_,c=this.mesh_.vertexArray_,e=this.mesh_.octree_.intersectSphere(this.interPoint_,a,this.mesh_.leavesUpdate_),e=this.mesh_.getVerticesFromTriangles(e),g=e.length,d=++Vertex.sculptMask_,h=this.pickedVertices_,f=0,k=f=
...
[e,g])},rotate:function(a,b,c,e,g,d,h,f){var k=this.rotateData_;f&&(k=this.rotateDataSym_);f=k.center2d;g=[e-f[0],g-f[1]];if(!(30>vec2.len(g))){vec2.normalize(g,g);k=k.normal;e=[0,0,0,0];d=[d-f[0],h-f[1]];vec2.normalize(d,d);d=Geometry.signedAngle2d(g,d);h=this.mesh_.vertexArray_;c=Math.sqrt(c);f=b.length;g=a[0];for(var l=a[1],m=a[2],n=0;n<f;++n){var p=3*b[n],q=h[p]-g,s=h[p+1]-l,r=h[p+2]-m,q=Math.sqrt(q*q+s*s+r*r)/c,s=[h[p],h[p+1],h[p+2]];quat.setAxisAngle(e,k,d*(3*q*q*q*q-4*q*q*q+1));vec3.sub(s,
s,a);vec3.transformQuat(s,s,e);vec3.add(s,s,a);h[p]=s[0];h[p+1]=s[1];h[p+2]=s[2]}}},smooth:function(a,b){var c=this.mesh_.vertexArray_,e=a.length,g=new Float32Array(3*e);this.laplacianSmooth(a,g);for(var d=this.d2Move_,h=Math.sqrt(d),f=this.topo_===Sculpt.topo.ADAPTIVE,k=0;k<e;++k){var l=3*a[k],m=3*k,n=(g[m]-c[l])*b,p=(g[m+1]-c[l+1])*b,m=(g[m+2]-c[l+2])*b;if(f){var q=n*n+p*p+m*m;q>d&&(q=h/Math.sqrt(q),n*=q,p*=q,m*=q)}c[l]+=n;c[l+1]+=p;c[l+2]+=m}},flatten:function(a,b,c,e,g){var d=this.areaNormal(c);
if(d){var h=this.areaCenter(c);c=this.mesh_.vertexArray_;e=Math.sqrt(e);var f=b.length;g*=0.3;var k=a[0],l=a[1];a=a[2];for(var m=h[0],n=h[1],h=h[2],p=d[0],q=d[1],d=d[2],s=Math.sqrt(this.d2Move_),r=this.topo_===Sculpt.topo.ADAPTIVE,t=0;t<f;++t){var v=3*b[t],u=c[v],w=c[v+1],y=c[v+2],x=(u-m)*p+(w-n)*q+(y-h)*d,u=u-k,w=w-l,y=y-a,y=Math.sqrt(u*u+w*w+y*y)/e,y=3*y*y*y*y-4*y*y*y+1,y=r?Math.min(s,x*g*y):x*g*y;c[v]-=p*y;c[v+1]-=q*y;c[v+2]-=d*y}}},pinch:function(a,b,c,e){var g=this.mesh_.vertexArray_;c=Math.sqrt(c);
var d=b.length,h=a[0],f=a[1];a=a[2];var k=Math.sqrt(this.d2Move_),l=this.topo_===Sculpt.topo.ADAPTIVE;e=0.005*e*c;for(var m=0;m<d;++m){var n=3*b[m],p=g[n],q=g[n+1],s=g[n+2],r=p-h,t=q-f,v=s-a,r=Math.sqrt(r*r+t*t+v*v)/c,r=3*r*r*r*r-4*r*r*r+1,r=l?Math.min(k,e*r):e*r;g[n]+=(h-p)*r;g[n+1]+=(f-q)*r;g[n+2]+=(a-s)*r}},crease:function(a,b,c,e,g){var d=this.areaNormal(c);if(d){c=this.mesh_.vertexArray_;e=Math.sqrt(e);var h=b.length,f=a[0],k=a[1];a=a[2];var l=d[0],m=d[1],d=d[2],n=Math.sqrt(this.d2Move_),p=this.topo_===
Sculpt.topo.ADAPTIVE;g=0.005*g*e;var q=10;this.negative_&&(q=-10);for(var s=0;s<h;++s){var r=3*b[s],t=c[r],v=c[r+1],u=c[r+2],w=t-f,y=v-k,x=u-a,w=Math.sqrt(w*w+y*y+x*x)/e,x=3*w*w*w*w-4*w*w*w+1,z=g*Math.pow(2-x,-20)*q,x=g*x,w=(f-t)*x+l*z,y=(k-v)*x+m*z,x=(a-u)*x+d*z;p&&(t=w*w+y*y+x*x,t>d2Move&&(t=n/Math.sqrt(t),w*=t,y*=t,x*=t));c[r]+=w;c[r+1]+=y;c[r+2]+=x}}},smoothFlat:function(a,b){var c=this.mesh_,e=c.vertexArray_,c=c.normalArray_,g=a.length,d=new Float32Array(3*g);this.laplacianSmooth(a,d);for(var h=
0;h<g;++h){var f=3*a[h],k=e[f],l=e[f+1],m=e[f+2],n=c[f],p=c[f+1],q=c[f+2],s=3*h,r=d[s],t=d[s+1],s=d[s+2],v=n*(r-k)+p*(t-l)+q*(s-m);e[f]+=(r-n*v-k)*b;e[f+1]+=(t-p*v-l)*b;e[f+2]+=(s-q*v-m)*b}},laplacianSmooth:function(a,b){for(var c=this.mesh_,e=c.vertices_,c=c.vertexArray_,g=a.length,d=0;d<g;++d){var h=3*d,f=e[a[d]],k=f.ringVertices_,l=k.length,m=0,n=0,p=0,q=0,s=0;if(l!==f.tIndices_.length){for(q=f=0;q<l;++q){var s=k[q],r=e[s];r.ringVertices_.length!==r.tIndices_.length&&(s*=3,m+=c[s],n+=c[s+1],p+=
c[s+2],++f)}b[h]=m/f;b[h+1]=n/f;b[h+2]=p/f}else{for(q=0;q<l;++q)s=3*k[q],m+=c[s],n+=c[s+1],p+=c[s+2];b[h]=m/l;b[h+1]=n/l;b[h+2]=p/l}}},areaNormal:function(a){for(var b=this.mesh_.normalArray_,c=a.length,e=0,g=0,d=0,h=0;h<c;++h)var f=3*a[h],e=e+b[f],g=g+b[f+1],d=d+b[f+2];a=Math.sqrt(e*e+g*g+d*d);if(0===a)return null;a=1/a;return[e*a,g*a,d*a]},areaCenter:function(a){for(var b=this.mesh_.vertexArray_,c=a.length,e=0,g=0,d=0,h=0;h<c;++h)var f=3*a[h],e=e+b[f],g=g+b[f+1],d=d+b[f+2];return[e/c,g/c,d/c]}};var Tools={nextHighestPowerOfTwo:function(a){--a;for(var b=1;32>b;b<<=1)a|=a>>b;return a+1},tidy:function(a){a.sort(function(a,b){return a-b});for(var b=a.length,c=0,e=0,c=1;c<b;++c)a[e]!==a[c]&&(a[++e]=a[c]);1<c&&(a.length=e+1)},intersectionArrays:function(a,b){for(var c=0,e=0,g=[],d=a.length,h=b.length;c<d&&e<h;)a[c]<b[e]?c++:a[c]>b[e]?e++:(g.push(a[c]),++c,++e);return g}},Tablet={};Tablet.plugin=document.querySelector("object[type='application/x-wacomtabletplugin']");
...
g.length;if(t){var w=d.length,l=new Vertex(w),u=3*b,m=f[u],q=f[u+1],t=f[u+2],y=k[u],x=k[u+1],z=k[u+2],u=3*c,D=f[u],F=f[u+1],A=f[u+2],B=k[u],C=k[u+1],G=k[u+2],u=y*B+x*C+z*G,H=0,H=-1>=u?Math.PI:1<=u?0:Math.acos(u),I=y+B,J=x+C,K=z+G,E=1/Math.sqrt(I*I+J*J+K*K),u=3*w;k[u]=I*E;k[u+1]=J*E;k[u+2]=K*E;w=m-D;I=q-F;J=t-A;E=Math.sqrt(w*w+I*I+J*J);0<w*(y-B)+I*(x-C)+J*(z-G)?(f[u]=0.5*(m+D)+0.12*k[u]*H*E,f[u+1]=0.5*(q+F)+0.12*k[u+1]*H*E,f[u+2]=0.5*(t+A)+0.12*k[u+2]*H*E):(f[u]=0.5*(m+D)-0.12*k[u]*H*E,f[u+1]=0.5*
(q+F)-0.12*k[u+1]*H*E,f[u+2]=0.5*(t+A)-0.12*k[u+2]*H*E);l.stateFlag_=Mesh.stateMask_;l.ringVertices_.push(b,c,e);n.replaceRingVertex(c,v);p.replaceRingVertex(b,v);l.tIndices_.push(a,s);d.push(l)}else b=d[v],b.ringVertices_.push(e),b.tIndices_.push(a,s);g.push(s);h.push(r)};
...
this.ctrlNegative_=this.ctrlSculpt_=this.ctrlShaders_=this.ctrlColor_=null;this.resetSphere_=this.resetSphere;this.open_=this.openFile;this.save_=this.saveFile;this.undo_=this.onUndo;this.redo_=this.onRedo;this.dummyFunc_=function(){}}SculptGL.elementIndexType=0;SculptGL.indexArrayType=Uint16Array;
...
c,b,a,this.lastMouseX_,this.lastMouseY_),this.symmetry_&&(this.pickingSym_.pickVerticesInSphere(this.pickingSym_.rWorldSqr_),this.sculpt_.sculptMesh(this.pickingSym_,c,this.lastMouseX_,this.lastMouseY_,b,a,!0))),this.mesh_.updateBuffers(),this.ctrlNbVertices_.name("Ver : "+this.mesh_.vertices_.length),this.ctrlNbTriangles_.name("Tri : "+this.mesh_.triangles_.length)):3===this.mouseButton_?this.camera_.rotate(b,a):2===this.mouseButton_&&this.camera_.translate((b-this.lastMouseX_)/3E3,(a-this.lastMouseY_)/
3E3);this.lastMouseX_=b;this.lastMouseY_=a;this.render()},sculptStroke:function(a,b,c,e){var g=this.ptPlane_,d=this.nPlane_,h=this.picking_,f=this.pickingSym_,k=a-this.lastMouseX_,l=b-this.lastMouseY_,m=Math.sqrt(k*k+l*l);this.sumDisplacement_+=m;var n=0.2*h.rDisplay_,p=m/Math.floor(m/n),k=k/m,l=l/m;a=this.lastMouseX_;b=this.lastMouseY_;var q=this.mesh_,s=this.symmetry_,r=this.sculpt_;if(this.sumDisplacement_>50*n)this.sumDisplacement_=0;else if(this.sumDisplacement_>n)for(n=this.sumDisplacement_=
0;n<m;n+=p){h.intersectionMouseMesh(q,a,b,c);if(!h.mesh_)break;h.pickVerticesInSphere(h.rWorldSqr_);r.sculptMesh(h,e);if(s){f.intersectionMouseMesh(q,a,b,c,g,d);if(!f.mesh_)break;f.rWorldSqr_=h.rWorldSqr_;f.pickVerticesInSphere(f.rWorldSqr_);r.sculptMesh(f,e)}a+=k*p;b+=l*p}},onMouseOut:function(){this.mouseButton_=0},onContextLost:function(){alert("shit happens : context lost")},onContextRestored:function(){alert("context is restored")},loadFile:function(a){a.stopPropagation();a.preventDefault();
if(0!==a.target.files.length&&(a=a.target.files[0],a.name.endsWith(".obj"))){var b=new FileReader,c=this;b.onload=function(a){c.startMeshLoad();Files.loadOBJ(a.target.result,c.mesh_);c.endMeshLoad();$("#fileopen").replaceWith($("#fileopen").clone(!0))};b.readAsText(a)}},resetSphere:function(){this.startMeshLoad();Files.loadOBJ(this.sphere_,this.mesh_);this.endMeshLoad()},startMeshLoad:function(){this.mesh_=new Mesh(this.gl_);this.states_.reset();this.states_.mesh_=this.mesh_;this.sculpt_.mesh_=this.mesh_;
Mesh.stateMask_=1;Vertex.tagMask_=1;Vertex.sculptMask_=1;Triangle.tagMask_=1},endMeshLoad:function(){var a=this.mesh_;a.render_.shaderType_=Render.mode.MATERIAL;a.initMesh(this.textures_,this.shaders_);a.moveTo([0,0,0]);a=vec3.dist(a.octree_.aabbLoose_.max_,a.octree_.aabbLoose_.min_);this.camera_.reset();this.camera_.globalScale_=a;this.camera_.zoom(-0.4);this.updateGuiMesh();this.render()},updateGuiMesh:function(){if(this.mesh_){var a=this.mesh_;this.ctrlColor_.object=a.render_;this.ctrlColor_.updateDisplay();
this.ctrlShaders_.object=a.render_;this.ctrlShaders_.updateDisplay();this.ctrlNbVertices_.name("Ver : "+a.vertices_.length);this.ctrlNbTriangles_.name("Tri : "+a.triangles_.length)}},openFile:function(){$("#fileopen").trigger("click")},saveFile:function(){if(this.mesh_){var a=[Files.exportOBJ(this.mesh_)],a=new Blob(a,{type:"text/plain;charset=utf-8"});saveAs(a,"yourMesh.obj")}},onUndo:function(){this.states_.undo();this.render();this.updateGuiMesh()},onRedo:function(){this.states_.redo();this.render();
this.updateGuiMesh()}};
